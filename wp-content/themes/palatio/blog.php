...
if ( function_exists( 'palatio_elementor_is_preview' ) && palatio_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			palatio_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	palatio_new_main_query(
		array(
			'post_type'      => palatio_get_theme_option( 'post_type' ),
			'category'       => palatio_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'palatio_filter_get_template_part', palatio_blog_archive_get_template() ) );
}
