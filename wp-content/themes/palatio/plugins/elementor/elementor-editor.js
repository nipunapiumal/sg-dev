...
jQuery( document ).ready(
	function() {
		...
		palatio_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="palatio_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.palatio_post_editor' );
						if ( link.length > 0 ) {
							palatio_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.palatio_post_editor', function(e) {
						palatio_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('palatio_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function palatio_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
