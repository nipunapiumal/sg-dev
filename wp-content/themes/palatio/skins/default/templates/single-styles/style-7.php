...
if ( apply_filters( 'palatio_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$palatio_with_featured_image = palatio_is_with_featured_image( $palatio_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $palatio_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $palatio_with_featured_image ) {
                echo ' with_featured_image' . ( false && palatio_get_theme_option( 'single_parallax' ) == 0 ? ' palatio-full-height' : '' );
            }
		?>">
			<?php
