...
if ( has_post_thumbnail() ) {
    $image = trx_addons_get_attachment_url(
        ...
        apply_filters('trx_addons_filter_thumb_size', palatio_get_thumb_size('masonry-big'), 'events-classic')
    );
}
    ...
    <div class="sc_events_item_content">
        ...
        <div class="sc_events_item_content_inner">
            ...
            <div class="sc_events_item_content_inner_top">
                <div class="sc_events_item_meta_categories"><?php palatio_show_layout(trx_addons_get_post_terms(' ', get_the_ID(), Tribe__Events__Main::TAXONOMY))?></div>
            </div>
            ...
            <div class="sc_events_item_content_inner_bottom">
                ...
                <div class="sc_events_item_meta">
                    <span class="sc_events_item_meta_item sc_events_item_meta_date"><?php
	                    ...
	                    $tm = tribe_get_start_time(null, get_option('time_format'));
                        $dt2 = tribe_get_end_date(null, true, 'Y-m-d');
	                    $tm2 = tribe_get_end_time(null, get_option('time_format'));
                        echo sprintf( $dt < date('Y-m-d') 
                                        ? esc_html__('Started on %1$s to %2$s', 'palatio') 
                                        : esc_html__('Starting %1$s to %2$s', 'palatio'),
                                    '<span class="sc_events_item_date sc_events_item_date_start">' . date_i18n(get_option('date_format'), strtotime($dt)) . ( $tm != null ? ' ' . $tm : '' ) . '</span>',
                                    '<span class="sc_events_item_date sc_events_item_date_end">' . date_i18n(get_option('date_format'), strtotime($dt2)) . ( $tm2 != null ? ' ' . $tm2 : '' ) . '</span>'
                                    );
                    ?></span>
                </div>
