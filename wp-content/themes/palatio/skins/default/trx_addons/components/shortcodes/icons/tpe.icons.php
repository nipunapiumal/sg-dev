...
var id = settings._element_id ? settings._element_id + '_sc' : 'sc_icons_'+(''+Math.random()).replace('.', ''),
    svg_bg_dir = '<?php echo trx_addons_get_svg_from_file(palatio_get_file_dir('images/decor.svg')); ?>',
    number = 0;
	...
	_.each(settings.icons, function(item) {
        ...
        #><div class="sc_icons_item<# if (item.link.url != '' ) print(' sc_icons_item_linked')
            ...
            if (item.char != '') {
                ...
                #><div class="sc_icons_image"><img src="{{ item.image.url }}" alt="<?php esc_attr_e('Icon', 'palatio'); ?>"></div><#
                    if ( settings.type == 'creative' && svg_bg_dir != '' )  {
                        #><div class="sc_icons_decoration_bg"><# print(svg_bg_dir); #></div><#
                    }
            } else {
                ...
                if (icon != '') {
                    ...
                    if (icon.indexOf('//') >= 0) {
                        if (icon.indexOf('.svg') >= 0) {
                            svg = icon;
                            item.icon_type = 'svg';
                            svg_id = id + '_' + ( '' + Math.random() ).replace( '.', '' );
                        } else {
                            img = icon;
                    ...
                    #><div <# if ( svg_id ) print( 'id="' + svg_id + '" ' ); #>class="sc_icons_icon sc_icon_type_{{ item.icon_type }} {{ icon }}"<#
						if (item_color != '') print(' style="color: ' + item_color + '"');
                    #>><#
                        if (svg != '') {
                            print( trx_addons_get_inline_svg( svg, {
								render: function( html ) {
									if ( html ) {
										elementor.$previewContents.find( '#' + svg_id ).html( html );
									}
								}
							} ) );
                        } else if (img != '') {
                            #><img class="sc_icon_as_image" src="{{ img }}" alt="<?php esc_attr_e('Icon', 'palatio'); ?>"><#
                        } else {
