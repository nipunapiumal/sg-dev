...
foreach ($args['values'] as $v) {
	...
	$icon_color_class = ! empty( $icon_color )
							? ' ' . ( strpos($icon, '.svg') !== false
										? trx_addons_add_inline_css_class( 'fill:' . esc_attr( $icon_color ), 'svg' )
				...
				. '<div class="sc_skills_item">'
					...
					. ( ! empty( $v['image'] )
						? '<div class="sc_skills_image">'
							. '<img src="' . esc_url( $v['image'] ) . '" alt="' . esc_attr__('Icon', 'palatio') . '"'
					...
					. ( ! empty( $icon ) && ! trx_addons_is_off( $icon )
						? '<div class="sc_skills_icon sc_icon_type_' . esc_attr($v['icon_type']) . ' ' . esc_attr($icon) . esc_attr($icon_color_class) . '">'
								. ( ! empty($svg)
									...
									: '')
								. ( ! empty($img)
									? '<img class="sc_icon_as_image" src="'.esc_url($img).'" alt="'.esc_attr__('Icon', 'palatio').'">'
									: '')
								. '</div>'
						: ''
						)
