		...
		data-details="<?php
			echo esc_attr( '<div class="post_details">'
								...
								. '<div class="post_description">'
									. $details
									. ( empty( $palatio_template_args['hide_excerpt'] )
										? '<div class="post_description_content">' . get_the_excerpt() . '</div>'
										: ''
										)
									. '<a href="' . esc_url( $link ) . '" class="theme_button post_readmore"><span class="post_readmore_label">' . esc_html__( 'Learn more', 'palatio' ) . '</span></a>'
								. '</div>'
							. '</div>'
						);
		?>"
	...
	trx_addons_get_template_part(
		...
		apply_filters(
			...
			array(
				...
				'hover'         => '!info',
				'thumb_bg'      => $use_masonry ? false : true,
				'link'          => $link,
				'thumb_size'    => palatio_get_thumb_size(
									$use_masonry
										? ( $args['columns'] > 2 ? 'masonry' : ($args['columns'] > 1 ? 'masonry-big' : 'full') )
										: ( $args['columns'] > 2 ? 'medium-square' : ($args['columns'] > 1 ? 'square' : 'full')
										)
									),
                ...
                'post_info'     => '<a href="' . esc_url( $link ) . '"></a>' . '<div class="post_info">'
                    . '<h5 class="post_title">'
                    . '<a href="' . esc_url( $link ) . '">' . palatio_strwords( esc_html( get_the_title() ), 10 ) . '</a>'
                    . '</h5>'
