...
while ( have_posts() ) { the_post();
	...
	?><article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>" <?php post_class( 'portfolio_page itemscope portfolio_page_details_'.esc_attr($meta['details_position']) . (in_array($meta['details_style'], array('light')) ? ' detail_light_style': ' detail_default_style') ); trx_addons_seo_snippets('', 'Article'); ?>><?php
							...
							foreach($images as $img) {
								...
								?>">
									<figure><?php
										...
										?><a href="<?php echo esc_url($img); ?>" title="<?php echo esc_attr($img_title); ?>"><img src="<?php echo esc_url($thumb); ?>" alt="<?php esc_attr_e('Gallery item', 'palatio'); ?>"<?php if (!empty($attr[3])) echo ' '.trim($attr[3]); ?>></a><?php
										if (!empty($img_title)) {
