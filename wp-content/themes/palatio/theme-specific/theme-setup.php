 ...
 *
 * @package PALATIO
 * @since PALATIO 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'PALATIO_THEME_FREE' ) ) {
	define( 'PALATIO_THEME_FREE', false );
}
if ( ! defined( 'PALATIO_THEME_FREE_WP' ) ) {
	define( 'PALATIO_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'PALATIO_THEME_ALLOW_UPDATE' ) ) {
	define( 'PALATIO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'PALATIO_ALLOW_SKINS' ) ) {
	define( 'PALATIO_ALLOW_SKINS', true );
}
if ( ! defined( 'PALATIO_DEFAULT_SKIN' ) ) {
	define( 'PALATIO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'PALATIO_REMEMBER_SKIN' ) ) {
	define( 'PALATIO_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'palatio_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function palatio_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = palatio_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'palatio_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'palatio_trx_updater_get_theme_info' );
	/**
	 * Add an information about the theme engine to the array with a theme info.
	 *
	 * @hooked 'trx_updater_filter_get_theme_info'
	 *
	 * @param array $info  An array with a theme information.
	 *
	 * @return array       A theme information with an engine info.
	 */
	function palatio_trx_updater_get_theme_info( $info ) {
		$engine = palatio_storage_get( 'engine' );
...
if ( ! function_exists( 'palatio_original_theme_slug' ) ) {
	add_filter( 'palatio_filter_original_theme_slug', 'palatio_original_theme_slug' );
	/**
...
if ( ! function_exists( 'palatio_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'palatio_importer_set_options', 9 );
	/**
	 * Set a theme-specific options for the import of the demo data via plugin ThemeREX Addons.
	 *
	 * @hooked 'trx_addons_filter_importer_options', 9
	 *
	 * @param array $options  Options for the import of the demo data.
	 *
	 * @return array          Processed options with a theme-specific values.
	 */
	function palatio_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( palatio_get_protocol() . ':' . palatio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( palatio_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'palatio_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'palatio_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function palatio_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( palatio_get_protocol() . ':' . palatio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'palatio_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'palatio_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function palatio_customizer_theme_setup1() {
		palatio_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'palatio_filter_allow_fullscreen' return false)
