<div class="front_page_section front_page_section_testimonials<?php
	$palatio_scheme = palatio_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $palatio_scheme ) && ! palatio_is_inherit( $palatio_scheme ) ) {
		echo ' scheme_' . esc_attr( $palatio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( palatio_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( palatio_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$palatio_css      = '';
		$palatio_bg_image = palatio_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$palatio_anchor_icon = palatio_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$palatio_anchor_text = palatio_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( palatio_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$palatio_css      = '';
			$palatio_bg_mask  = palatio_get_theme_option( 'front_page_testimonials_bg_mask' );
			$palatio_bg_color_type = palatio_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $palatio_bg_color_type ) {
				$palatio_bg_color = palatio_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$palatio_caption = palatio_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $palatio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $palatio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $palatio_caption, 'palatio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$palatio_description = palatio_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $palatio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $palatio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $palatio_description ), 'palatio_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! palatio_exists_trx_addons() ) {
						...
						palatio_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
