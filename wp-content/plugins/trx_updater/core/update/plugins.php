...
class Plugins extends Base {
	...
	public function __construct( $manager ) {
		...
		add_filter( 'update_bulk_plugins_complete_actions', array( $this, 'after_plugin_upgrade' ), 10, 2 );
		...
		if ( get_option('revslider-valid', 'false') == 'false' ) {
			add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'fix_revslider_save_plugin_package' ), 1 );
			add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'fix_revslider_restore_plugin_package' ), 9999 );
		}
		
		// Fix for Booked (old versions 2.4.3-)
		add_action( 'init', array( $this, 'fix_booked_disable_clear_update_info' ), 9999 );
	}
	...
	private function get_plugins_info( $required_plugins ) {
		...
		if ( is_array( $required_plugins ) ) {
			...
			if ( ! is_array( $data ) || ! empty( $_GET['force-check'] ) ) {
				...
				set_transient( 'trx_updater_plugins_info', $data, 2 * 24 * 60 * 60 );       // Store to the cache for 2 days
			}
			if ( is_array($data) ) {
				foreach ( $required_plugins as $k => $v ) {
					$slug    = $k;
					...
					if ( ! empty( $title )
						// His version greater or equal to version in the field 'update_from' in the plugin's data received from the upgrade server
						// or if the field 'update_from' is absent ( any version can be updated )
						&& ! empty( $data[$slug]['version'] )
						&& ( empty( $data[$slug]['update_from'] ) || version_compare( $version, $data[$slug]['update_from'], '>=' ) )
						// A current plugin is not 'trx_addons' or an archive 'trx_addons.zip' is absent in the theme folder (was removed from the theme core v.1.74.0+)
						&& ( $slug != 'trx_addons'
							|| (   ! file_exists( $this->theme_dir . '/plugins/trx_addons/trx_addons.zip' )
								&& ! file_exists( $this->theme_dir . '/plugins/install/trx_addons.zip' )
								)
							)
						// A current plugin is not 'trx_addons' or a function '{{theme_slug}}_trx_addons_action_page_content_wrap' exists (was added to the theme core v.1.74.0+)
						&& ( $slug != 'trx_addons'
							|| function_exists( str_replace( '-', '_', (string)$this->theme_slug ) . '_trx_addons_action_page_content_wrap' )
							)
						// A current plugin is not 'trx_addons' or a function 'trx_addons_add_inline_styles' is not exists (was removed in the v.1.73.0+)
						&& ( $slug != 'trx_addons'
							|| ! function_exists( 'trx_addons_add_inline_styles' )
							)
					) {
						$info[$k] = array(
							'title'   => $title,
	 ...
	 * Plugin's update URL
	 *
	 * Save plugin's update URL (fix for Slider Revolution)
	 *
	 * @since 1.9.6
	 * @access private
	 *
	 * @var theme_parts
	 */
	private $revslider_plugin_package;
	
	/**
	 * Slider Revolution fix: Save a plugin's package URL
	 *
	 * Save a plugin's package URL
	 *
	 * @hook 'pre_set_site_transient_update_plugins', 1
	 ...
	 * @access public
	 */
	public function fix_revslider_save_plugin_package( $transient ) {
		if ( ! empty( $transient->response['revslider/revslider.php']->package ) ) {
			$this->revslider_plugin_package = $transient->response['revslider/revslider.php']->package;
		}
		return $transient;
	}
	 ...
	 * @access public
	 */
	public function fix_revslider_restore_plugin_package( $transient ) {
		if ( ! empty( $transient->response['revslider/revslider.php'] )
				&& empty( $transient->response['revslider/revslider.php']->package )
				&& ! empty( $this->revslider_plugin_package )
		) {
			$transient->response['revslider/revslider.php']->package = $this->revslider_plugin_package;
		}
		return $transient;
	}
