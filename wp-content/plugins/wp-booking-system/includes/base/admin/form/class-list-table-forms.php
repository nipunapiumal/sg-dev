...
Class WPBS_WP_List_Table_Forms extends WPBS_WP_List_Table {
    ...
    protected function get_views() {
    	...
    	$form_status = ( ! empty( $_GET['form_status'] ) ? esc_attr( $_GET['form_status'] ) : 'active' );

        $views = array(
            'active' => '<a href="' . add_query_arg( array( 'page' => 'wpbs-forms', 'form_status' => 'active', 'paged' => 1, 's' => ( ! empty( $_GET['s'] ) ? esc_attr( $_GET['s'] ) : '' ) ), admin_url( 'admin.php' ) ) . '" ' . ( $form_status == 'active' ? 'class="current"' : '' ) . '>' . __( 'Active', 'wp-booking-system' ) . ' <span class="count">(' . wpbs_get_forms( array( 'status' => 'active', 'search'  => ( ! empty( $_GET['s'] ) ? esc_attr( $_GET['s'] ) : '' ) ), true ) . ')</span></a>',
            'trash'  => '<a href="' . add_query_arg( array( 'page' => 'wpbs-forms', 'form_status' => 'trash', 'paged' => 1, 's' => ( ! empty( $_GET['s'] ) ? esc_attr( $_GET['s'] ) : '' ) ), admin_url( 'admin.php' ) ) . '" ' . ( $form_status == 'trash' ? 'class="current"' : '' ) . '>' . __( 'Trash', 'wp-booking-system' ) . ' <span class="count">(' . wpbs_get_forms( array( 'status' => 'trash', 'search'  => ( ! empty( $_GET['s'] ) ? esc_attr( $_GET['s'] ) : '' ) ), true ) . ')</span></a>',
        );
