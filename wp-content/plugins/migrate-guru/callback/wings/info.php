...
class MGInfoCallback extends MGCallbackBase {
	...
	public function getSystemInfo() {
		$sys_info = array(
			'host' => MGHelper::getRawParam('SERVER', 'HTTP_HOST'),
			'phpversion' => phpversion(),
			'AF_INET6' => defined('AF_INET6')
		);
		if (array_key_exists('SERVER_ADDR', $_SERVER)) {
			$sys_info['serverip'] = MGHelper::getRawParam('SERVER', 'SERVER_ADDR');
		}
	...
	public function getHostInfo() {
		...
		if (isset($_SERVER['SERVER_ADDR']) && function_exists('gethostbyaddr')) {
			$host_info['HOST_FROM_IP'] = gethostbyaddr(MGHelper::getRawParam('SERVER', 'SERVER_ADDR'));
		}
	...
	public function serverConfig() {
		return array(
			'software' => MGHelper::getRawParam('SERVER', 'SERVER_SOFTWARE'),
