...
if (!class_exists('MGRespStream')) :

	class MGStream extends MGCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new MGRespStream($request);
			if ($request->isAPICall()) {
				$stream = new MGHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class MGRespStream extends MGStream {
	public $bvboundry;
...
class MGHttpStream extends MGStream {
	var $user_agent = 'MGHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
