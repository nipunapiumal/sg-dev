...
if (!class_exists('MGHelper')) :
	class MGHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!MGWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = MGWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					MGWPFileSystem::getInstance()->putContents($fname, $modified_content,
							MGWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
