		...
		_.each(settings.links, function(item) {
			...
			#>"><#
				...
				if (item.url.url == '') {
					...
					var link_atts = trx_addons_get_link_attributes( item.url );
					#><a class="custom_links_list_item_link" href="{{ item.url.url }}"{{{ link_atts }}}<#
						if (item.color!='') print(' style="color:'+item.color+' !important;"');
					#>><#
				}
				// Image or Icon
				...
				if (item.caption != '') {
					var link_atts = trx_addons_get_link_attributes( item.url );
					#><a class="custom_links_list_item_button sc_button sc_button_simple" href="{{ item.url.url }}"{{{ link_atts }}}><#
						print(item.caption); 
					#></a><#
				}
			#></li><#
