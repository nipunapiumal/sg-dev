...
class trx_addons_widget_categories_list extends TRX_Addons_Widget {
	...
	function widget($args, $instance) {
		...
		$instance['cat_order'] = isset($instance['cat_order']) ? $instance['cat_order'] : '';
		...
		// Sort categories by slug according to the order in the $instance['cat_order']
		if ( ! empty( $instance['cat_order'] ) ) {
			$cat_order = explode( ',', $instance['cat_order'] );
			if ( is_array( $cat_order ) && count( $cat_order ) > 0 ) {
				$categories_sorted = array();
				foreach( $cat_order as $cat_slug ) {
					foreach( $categories as $k => $cat ) {
						if ( $cat->term_id == (int)$cat_slug || urldecode( $cat->slug ) == $cat_slug ) {
							$categories_sorted[] = $cat;
							unset( $categories[$k] );
							break;
						}
					}
				}
				$categories = array_merge( $categories_sorted, $categories );
			}
		}
		...
		trx_addons_get_template_part( array(
										TRX_ADDONS_PLUGIN_WIDGETS . 'categories_list/tpl.categories-list-' . trim( trx_addons_sanitize_file_name( $instance['style'] ) ) . '.php',
	...
	function update($new_instance, $instance) {
		...
		$instance['cat_order'] = ! empty( $new_instance['cat_order'] )
									? strip_tags( $new_instance['cat_order'] )
									: ( ! empty( $new_instance['cat_list'] ) && is_string( $new_instance['cat_list'] )
	...
	function form($instance) {
		...
		$this->show_field(array('name' => 'cat_order',
								'title' => __( 'Order', 'trx_addons' ),
								'desc' => __( 'Comma separated list of category slugs. If empty - categories will be shown in the alphabet order.', 'trx_addons' ),
								'value' => $instance['cat_order'],
								'type' => 'text'));
