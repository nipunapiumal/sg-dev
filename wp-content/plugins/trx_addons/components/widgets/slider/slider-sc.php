...
if ( !function_exists( 'trx_addons_sc_widget_slider' ) ) {
	function trx_addons_sc_widget_slider($atts, $content=''){	
		$atts = trx_addons_sc_prepare_atts( 'trx_widget_slider', $atts, trx_addons_sc_common_atts( 'trx_widget_slider', 'id', array(
			...
			'slides_per_view' => '1',
			'slides_per_view_widescreen' => '',
			'slides_per_view_desktop' => '',
			'slides_per_view_laptop' => '',
			'slides_per_view_tablet_extra' => '',
			'slides_per_view_tablet' => '',
			'slides_per_view_mobile_extra' => '',
			'slides_per_view_mobile' => '',
			'slides_space' => '0',
			'slides_space_widescreen' => '',
			'slides_space_desktop' => '',
			'slides_space_laptop' => '',
			'slides_space_tablet_extra' => '',
			'slides_space_tablet' => '',
			'slides_space_mobile_extra' => '',
			'slides_space_mobile' => '',
			'slides_min_width' => '',
			'slides_min_width_widescreen' => '',
			'slides_min_width_desktop' => '',
			'slides_min_width_laptop' => '',
			'slides_min_width_tablet_extra' => '',
			'slides_min_width_tablet' => '',
			'slides_min_width_mobile_extra' => '',
			'slides_min_width_mobile' => '',
			'slides_parallax' => '0',
			'slides_type' => 'bg',
			'slides_ratio' => '16:9',
			"slides_centered" => '0',
			...
			'controller_icon' => '',
			'controller_icon_prev' => '',
			'controller_icon_next' => '',
			...
			'controller_per_view_widescreen' => '',
			'controller_per_view_desktop' => '',
			'controller_per_view_laptop' => '',
			'controller_per_view_tablet_extra' => '',
			'controller_per_view_tablet' => '',
			'controller_per_view_mobile_extra' => '',
			'controller_per_view_mobile' => '',
			'controller_space' => 0,		// Space between slides in the controller
			'controller_space_widescreen' => '',
			'controller_space_desktop' => '',
			'controller_space_laptop' => '',
			'controller_space_tablet_extra' => '',
			'controller_space_tablet' => '',
			'controller_space_mobile_extra' => '',
			'controller_space_mobile' => '',
...
if ( !function_exists( 'trx_addons_sc_slider_controller' ) ) {
	function trx_addons_sc_slider_controller($atts, $content=''){	
		...
		$allow_sc_styles_in_elementor = apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'sc_slider_controls' );
		...
		$output = '<div' . (!empty($atts['id']) ? ' id="'.esc_attr($atts['id']).'"' : '')
						...
						. ' data-controls-icon="'.esc_attr($atts['icon']).'"'
						. ( $allow_sc_styles_in_elementor
							? ' data-controls-icon-prev="' . esc_attr( trx_addons_elm_render_icon( $atts['arrows_icon_prev'], 'eicon-chevron-left', false ) ) . '"'
								. ' data-controls-icon-next="' . esc_attr( trx_addons_elm_render_icon( $atts['arrows_icon_next'], 'eicon-chevron-right', false ) ) . '"'
							: ''
							)
...
if ( !function_exists( 'trx_addons_sc_slider_controls' ) ) {
	function trx_addons_sc_slider_controls($atts, $content = ''){	
		$atts = trx_addons_sc_prepare_atts( 'trx_slider_controls', $atts, trx_addons_sc_common_atts( 'trx_slider_controls', 'id', array(
			...
			'arrows_icon_prev' => '',
			'arrows_icon_next' => '',
			'pagination_title_html_tag' => 'span',
			'pagination_title_orientation' => 'vertical'
		) ) );

		$allow_sc_styles_in_elementor = apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'sc_slider_controls' );

		$arrow_prev = ! empty( $allow_sc_styles_in_elementor ) ? '' : ( ! empty( $atts['icon'] ) ? trim( $atts['icon'] ) : 'slider_arrow_default' );
		...
		$output = '<div' . (!empty($atts['id']) ? ' id="'.esc_attr($atts['id']).'"' : '')
						. ' class="sc_slider_controls'
							...
							. ( $atts['pagination_style'] == 'titles'
								? ' slider_pagination_titles_' . ( ! empty( $atts['pagination_title_orientation'] ) ? esc_attr( $atts['pagination_title_orientation'] ) : 'vertical' )
								: ''
								)
						...
						. ' data-pagination-html-tag="'.esc_attr($atts['pagination_title_html_tag']).'"'
						. ($atts['css'] ? ' style="'.esc_attr($atts['css']).'"' : '')
					. '>'
						...
						.'">'
							. ( empty($atts['slider_id'])
								...
								: ( (empty($atts['hide_prev']) 
										? '<a class="slider_prev' . ( ! empty( $atts['title_prev'] ) ? ' with_title' : '' ) . ' ' . esc_attr( $arrow_prev ) . '" href="#" role="button">'
												. ( $allow_sc_styles_in_elementor && ! empty( $atts['arrows_icon_prev'] )
														? '<span class="slider_prev_icon trx-addons-icon">'
															. trx_addons_elm_render_icon( $atts['arrows_icon_prev'], 'eicon-chevron-left', false )
															. '</span>'
														: ''
														)
									...
									. (empty($atts['hide_next']) 
										? '<a class="slider_next' . ( ! empty( $atts['title_next'] ) ? ' with_title' : '' ) . ' ' . esc_attr( $arrow_next ) . '" href="#" role="button">'
												. ( $allow_sc_styles_in_elementor && ! empty( $atts['arrows_icon_next'] )
														? '<span class="slider_prev_icon trx-addons-icon">'
															. trx_addons_elm_render_icon( $atts['arrows_icon_next'], 'eicon-chevron-right', false )
															. '</span>'
														: ''
														)
