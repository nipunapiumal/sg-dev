...
?>>
	...
	<div class="sc_services_item_header"<?php if (!empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		...
		if ( ! empty( $link ) ) {
			?><a class="post_link sc_services_item_link" href="<?php echo esc_url( $link ); ?>"<?php if (!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) echo trx_addons_external_links_target(); ?>></a><?php
		}
