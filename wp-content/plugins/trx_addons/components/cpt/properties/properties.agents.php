...
if ( ! function_exists( 'trx_addons_cpt_agents_init' ) ) {
	...
	/**
	 * Register a post type and corresponding taxonomies.
	 *
	 * @hooked 'init'
	 */
		...
		register_taxonomy(
			...
			apply_filters('trx_addons_filter_register_taxonomy',
				array(
					...
					'query_var'         => true,
					'map_meta_cap'		=> true,
					'capability_type'   => 'properties_agent',
					'rewrite'           => array(
...
if ( !function_exists( 'trx_addons_properties_add_agents_to_team_list' ) ) {
	add_filter( 'trx_addons_filter_get_list_team_posts_types', 'trx_addons_properties_add_agents_to_team_list' );
	function trx_addons_properties_add_agents_to_team_list( $list ) {
		return array_merge( $list, array( TRX_ADDONS_CPT_AGENTS_PT => __( 'Property Agents', 'trx_addons' ) ) );
	}
}
...
if ( !function_exists( 'trx_addons_cpt_agents_single_template' ) ) {
	...
	function trx_addons_cpt_agents_single_template($template) {
		global $post;
		if (trx_addons_is_single() && $post->post_type == TRX_ADDONS_CPT_AGENTS_PT)
...
if (!function_exists('trx_addons_cpt_agents_taxonomy_save_custom_fields')) {
	add_action('edited_'.TRX_ADDONS_CPT_AGENTS_TAXONOMY,	'trx_addons_cpt_agents_taxonomy_save_custom_fields', 10, 1 );
	add_action('created_'.TRX_ADDONS_CPT_AGENTS_TAXONOMY,	'trx_addons_cpt_agents_taxonomy_save_custom_fields', 10, 1 );
	function trx_addons_cpt_agents_taxonomy_save_custom_fields($term_id) {
		if (isset($_POST['trx_addons_image'])) {
			trx_addons_set_term_meta(array(
											'term_id' => $term_id,
											'key' => TRX_ADDONS_CPT_PROPERTIES_TAXONOMY_IMAGE_KEY
											),
...
if (!function_exists('trx_addons_cpt_agents_taxonomy_show_custom_fields')) {
	add_action(TRX_ADDONS_CPT_AGENTS_TAXONOMY.'_edit_form_fields', 'trx_addons_cpt_agents_taxonomy_show_custom_fields', 10, 1 );
	add_action(TRX_ADDONS_CPT_AGENTS_TAXONOMY.'_add_form_fields', 'trx_addons_cpt_agents_taxonomy_show_custom_fields', 10, 1 );
	function trx_addons_cpt_agents_taxonomy_show_custom_fields($term) {
		...
		$img = $term_id > 0 ? trx_addons_get_term_image( $term_id, TRX_ADDONS_CPT_AGENTS_TAXONOMY, TRX_ADDONS_CPT_PROPERTIES_TAXONOMY_IMAGE_KEY ) : '';
...
if (!function_exists('trx_addons_cpt_agents_taxonomy_add_custom_column')) {
	add_filter('manage_edit-'.TRX_ADDONS_CPT_AGENTS_TAXONOMY.'_columns',	'trx_addons_cpt_agents_taxonomy_add_custom_column', 9);
	function trx_addons_cpt_agents_taxonomy_add_custom_column( $columns ){
...
if (!function_exists('trx_addons_cpt_agents_taxonomy_fill_custom_column')) {
	add_action('manage_'.TRX_ADDONS_CPT_AGENTS_TAXONOMY.'_custom_column',	'trx_addons_cpt_agents_taxonomy_fill_custom_column', 9, 3);
	function trx_addons_cpt_agents_taxonomy_fill_custom_column($output='', $column_name='', $term_id=0) {
...
// Add users role and capabilities for "Properties Agents"
if (!function_exists('trx_addons_cpt_agents_add_roles_and_caps')) {
	add_action( 'trx_addons_action_add_roles_and_caps', 'trx_addons_cpt_agents_add_roles_and_caps' );
	function trx_addons_cpt_agents_add_roles_and_caps() {
		// Create a new role
		add_role(
			'trx_addons_properties_agent',
			esc_html__( 'Properties Agent', 'trx_addons' ),
			array(
				'read'         => true,
				'edit_posts'   => false,
				'upload_files' => true,
		...
		trx_addons_add_capabilities(
			...
			array( 'properties_agent' )
		);
		trx_addons_add_capabilities(
			array( 'trx_addons_properties_agent' ),
			array( 'property' ),
		...
		trx_addons_add_capabilities(
			array( 'trx_addons_properties_agent' ),
			array( 'properties_agent' ),
			array( '_others', 'delete_', 'manage_%s_terms', 'edit_%s_terms', 'delete_%s_terms' )
		);
	}
}

// Allow access to admin area for "Properties Agents" 
if ( ! function_exists( 'trx_addons_cpt_agents_allow_admin_access' ) ) {
	add_filter( 'trx_addons_filter_allow_admin_access', 'trx_addons_cpt_agents_allow_admin_access', 10, 2 );
	function trx_addons_cpt_agents_allow_admin_access( $allow, $roles ) {
		if ( ! $allow && is_array( $roles ) && in_array( 'trx_addons_properties_agent', $roles ) ) {
			$allow = true;
		}
		return $allow;
	}
...
// Display only own posts for users 'properties_agent'
if ( ! function_exists( 'trx_addons_cpt_agents_disallow_other_posts_for_agents' ) ) {
	add_filter( 'pre_get_posts', 'trx_addons_cpt_agents_disallow_other_posts_for_agents' );
	function trx_addons_cpt_agents_disallow_other_posts_for_agents( $query ) {
		global $pagenow;
		if ( ! empty( $pagenow ) && 'edit.php' == $pagenow && ! empty( $query->is_admin ) ) {// && $query->is_main_query()
			$post_type = $query->get('post_type');
			if ( in_array( $post_type, array( TRX_ADDONS_CPT_PROPERTIES_PT, TRX_ADDONS_CPT_AGENTS_PT ) ) ) {
				if ( ( $post_type == TRX_ADDONS_CPT_PROPERTIES_PT && ! current_user_can( sprintf( 'edit_others_%ss', 'property' ) ) && trx_addons_is_on( trx_addons_get_option('agents_hide_others_properties') ) )
					||
					( $post_type == TRX_ADDONS_CPT_AGENTS_PT && ! current_user_can( sprintf( 'edit_others_%ss', 'properties_agent' ) ) )
				) {
//					global $user_ID;
...
if ( ! function_exists( 'trx_addons_cpt_agents_localize_admin_scripts' ) ) {
	add_filter( 'trx_addons_filter_localize_script_admin',	'trx_addons_cpt_agents_localize_admin_scripts');
	function trx_addons_cpt_agents_localize_admin_scripts( $vars = array() ) {
		$vars['hide_add_new_agent'] = ! current_user_can( sprintf( 'edit_others_%ss', 'properties_agent' ) );
		return $vars;
	}
}

// Remove the item 'Agents - Add New' for users 'properties_agent'
// Commented, because in the admin menu 'Agents' only two items now: 'All agents' and 'Add new'
// If 'Add new' is removed - submenu disappear and page 'All agents' is not available
// Removing the item 'Add new' is moved to js
/*
if ( ! function_exists( 'trx_addons_cpt_agents_remove_add_new_for_agents' ) ) {
	add_action( 'admin_menu', 'trx_addons_cpt_agents_remove_add_new_for_agents', 9999 );
	function trx_addons_cpt_agents_remove_add_new_for_agents() {
		if ( ! current_user_can( sprintf( 'edit_others_%ss', 'properties_agent' ) ) ) {
			remove_submenu_page( 'edit.php?post_type=' . TRX_ADDONS_CPT_AGENTS_PT, 'post-new.php?post_type=' . TRX_ADDONS_CPT_AGENTS_PT );
		}
	}
}
*/

// Disable URL post-new.php?post_type=cpt_agents for users 'properties_agent'
if ( ! function_exists( 'trx_addons_cpt_agents_disable_post_new' ) ) {
	add_action( 'init', 'trx_addons_cpt_agents_disable_post_new' );
	function trx_addons_cpt_agents_disable_post_new() {
		global $pagenow, $post_type;
		if ( is_admin()
			&& ! current_user_can( sprintf( 'edit_others_%ss', 'properties_agent' ) )
			&& ! empty( $pagenow )
			&& $pagenow == 'post-new.php'
			&& trx_addons_get_value_gp('post_type') == TRX_ADDONS_CPT_AGENTS_PT
		) {
			nocache_headers();
			if ( wp_safe_redirect( admin_url( 'edit.php?post_type=' . TRX_ADDONS_CPT_AGENTS_PT ) ) ) {
				exit;
			}
		}
	}
...
// Add "Property Agents" to the authors list
if ( ! function_exists( 'trx_addons_cpt_agents_add_to_authors_list' ) ) {
//	add_filter( 'quick_edit_dropdown_authors_args',	'trx_addons_cpt_agents_add_to_authors_list', 10, 2 );
	add_filter( 'wp_dropdown_users_args', 'trx_addons_cpt_agents_add_to_authors_list', 10, 2 );
	function trx_addons_cpt_agents_add_to_authors_list( $users_args, $params = false ) {
		global $pagenow, $post_type;
		if ( is_admin()
			&& current_user_can( sprintf( 'edit_others_%ss', 'properties_agent' ) )
			&& ! empty( $pagenow )
			&& (
				( in_array( $pagenow, array( 'edit.php', 'post-new.php' ) ) && trx_addons_get_value_gp('post_type') == TRX_ADDONS_CPT_AGENTS_PT )
				||
				( in_array( $pagenow, array( 'post.php' ) ) && trx_addons_get_value_gp('action') == 'edit' && ! empty( $post_type ) && $post_type == TRX_ADDONS_CPT_AGENTS_PT )
				)
		) {
			...
			$users_args['role__in'] = array_merge(
										$users_args['role__in'],
										array( 'trx_addons_properties_agent', 'administrator', 'editor' )
									);
			$users_args['who'] = '';
		}
		return $users_args;
...
// Add Capabilities to User Roles of 'trx_addons_properties_agent' to make it equal to 'Booked Agent'
if ( ! function_exists( 'trx_addons_cpt_agents_add_role_to_booked' ) ) {
	add_filter( 'booked_user_roles', 'trx_addons_cpt_agents_add_role_to_booked', 10, 1 );
	function trx_addons_cpt_agents_add_role_to_booked( $roles ) {
		$roles[] = 'trx_addons_properties_agent';
		return $roles;
	}
}

// Add a role 'booked_agent' to a current_user (if it is equal to  'trx_addons_properties_agent')
if ( ! function_exists( 'trx_addons_cpt_agents_add_booked_role' ) ) {
	add_action( 'init', 'trx_addons_cpt_agents_add_booked_role' );
	function trx_addons_cpt_agents_add_booked_role() {
		if ( is_user_logged_in() ) {
			$user = wp_get_current_user();
			if ( ! empty( $user->roles )
				&& is_array( $user->roles )
				&& in_array( 'trx_addons_properties_agent', $user->roles )
