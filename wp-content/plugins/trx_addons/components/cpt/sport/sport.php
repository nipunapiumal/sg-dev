		...
		<div id="trx_addons_sport_breadcrumbs" class="notice notice-info">
			<h3 class="trx_addons_sport_breadcrumbs_title">
				...
				if ($screen->post_type == TRX_ADDONS_CPT_COMPETITIONS_PT) {
					...
					if ( (int) $competition == 0) $competition = is_object($round) ? $round->post_parent : ( is_object( $post ) ? $post->post_parent : 0 );
...
// Delete all competitions, rounds, matches and players on the sport deletion
if ( ! function_exists( 'trx_addons_cpt_sport_delete_sport' ) ) {
	add_action( 'pre_delete_term', 'trx_addons_cpt_sport_delete_sport', 10, 2 );
	function trx_addons_cpt_sport_delete_sport( $term_id, $taxonomy ) {
		if ( $taxonomy !== TRX_ADDONS_CPT_COMPETITIONS_TAXONOMY ) {
			return;
		}
		// Get all competitions with this sport
		...
		// Loop through competitions and delete them with all children posts
...
// Delete all rounds, matches and players on the competition deletion
if ( ! function_exists( 'trx_addons_cpt_sport_delete_competition' ) ) {
	add_action( 'delete_post', 'trx_addons_cpt_sport_delete_competition', 10, 2 );
	function trx_addons_cpt_sport_delete_competition( $post_id, $post ) {
		if ( empty( $post->post_type ) || $post->post_type != TRX_ADDONS_CPT_COMPETITIONS_PT ) {
			return;
		}
		// Delete players
		trx_addons_cpt_sport_delete_children_posts( $post_id, TRX_ADDONS_CPT_PLAYERS_PT );
		// Delete rounds
		trx_addons_cpt_sport_delete_children_posts( $post_id, TRX_ADDONS_CPT_ROUNDS_PT );
	}
}

// Delete all matches on the round deletion
if ( ! function_exists( 'trx_addons_cpt_sport_delete_round' ) ) {
	add_action( 'delete_post', 'trx_addons_cpt_sport_delete_round', 10, 2 );
	function trx_addons_cpt_sport_delete_round( $post_id, $post ) {
		if ( empty( $post->post_type ) || $post->post_type != TRX_ADDONS_CPT_ROUNDS_PT ) {
			return;
		}
		// Delete matches
		trx_addons_cpt_sport_delete_children_posts( $post_id, TRX_ADDONS_CPT_MATCHES_PT );
	}
}

if ( ! function_exists( 'trx_addons_cpt_sport_delete_children_posts' ) ) {
	function trx_addons_cpt_sport_delete_children_posts( $parent_id, $post_type ) {
		...
		if ( ! empty( $children_posts ) && is_array( $children_posts ) ) {
			foreach ( $children_posts as $child_id ) {
