	...
	function trx_addons_cpt_cars_init() {
		...
		trx_addons_meta_box_register(TRX_ADDONS_CPT_CARS_PT, array(
			...
			"agent_type" => array(
				...
				"desc" => wp_kses_data( __("Select the type of content to be displayed in the contacts block: agent, post author or car owner/seller data.", 'trx_addons') ),
				"std" => "owner",
				"options" => array(
					"agent" => esc_html__('Agent', 'trx_addons'),
					"author" => esc_html__('Author', 'trx_addons'),
					"owner" => esc_html__('Car owner/seller', 'trx_addons')
				),
				"type" => "radio"
			),
			"agent" => array(
			...
			"owner_name" => array(
				...
				"desc" => wp_kses_data( __('Name of the car owner/seller', 'trx_addons') ),
				"dependency" => array(
			...
			"owner_phone" => array(
				...
				"desc" => wp_kses_data( __('Phone of the car owner/seller', 'trx_addons') ),
				"dependency" => array(
					"agent_type" => array("owner")
				),
				"class" => "trx_addons_column-1_3",
			...
			"owner_email" => array(
				...
				"desc" => wp_kses_data( __('E-mail of the car owner/seller', 'trx_addons') ),
				"dependency" => array(
					"agent_type" => array("owner")
				),
				"class" => "trx_addons_column-1_3",
...
// Check if the current page is a Search page for 'Cars' post type
if ( ! function_exists( 'trx_addons_cpt_cars_is_search_page' ) ) {
	add_filter( 'trx_addons_is_search_page', 'trx_addons_cpt_cars_is_search_page' );
	function trx_addons_cpt_cars_is_search_page( $is_search ) {
		if ( ! $is_search ) {
			$is_search = isset( $_GET['cars_query'] ) && isset( $_GET['cars_keywords'] );
		}
		return $is_search;
	}
}
...
if ( ! function_exists( 'trx_addons_cpt_cars_get_list_transmission' ) ) {
	function trx_addons_cpt_cars_get_list_transmission() {
		$options = trx_addons_get_option( 'cars_transmission_list' );
		if ( empty( $options ) || ! is_array( $options ) ) {
			$options = trx_addons_cpt_cars_get_default_transmission_options();
		}
		$list = array();
		foreach ( $options as $opt ) {
			if ( ! empty( $opt['slug'] ) && ! empty( $opt['title'] ) ) {
				$list[ $opt['slug'] ] = $opt['title'];
			}
		}
		return apply_filters( 'trx_addons_filter_cars_transmission', $list );
	}
}
...
if ( ! function_exists( 'trx_addons_cpt_cars_get_list_fuel' ) ) {
	function trx_addons_cpt_cars_get_list_fuel() {
		$options = trx_addons_get_option( 'cars_fuel_list' );
		if ( empty( $options ) || ! is_array( $options ) ) {
			$options = trx_addons_cpt_cars_get_default_fuel_options();
		}
		$list = array();
		foreach ( $options as $opt ) {
			if ( ! empty( $opt['slug'] ) && ! empty( $opt['title'] ) ) {
		...
		return apply_filters('trx_addons_filter_cars_fuel', $list );
	}
}
...
if ( ! function_exists( 'trx_addons_cpt_cars_get_list_type_of_drive' ) ) {
	function trx_addons_cpt_cars_get_list_type_of_drive() {
		$options = trx_addons_get_option( 'cars_type_of_drive_list' );
		if ( empty( $options ) || ! is_array( $options ) ) {
			$options = trx_addons_cpt_cars_get_default_type_of_drive_options();
		}
		$list = array();
		foreach ( $options as $opt ) {
			if ( ! empty( $opt['slug'] ) && ! empty( $opt['title'] ) ) {
		...
		return apply_filters('trx_addons_filter_cars_type_of_drive', $list );
	}
}

if ( ! function_exists( 'trx_addons_cpt_cars_fix_option_names' ) ) {
	add_action( 'trx_addons_action_is_new_version_of_plugin', 'trx_addons_cpt_cars_fix_option_names', 10, 2 );
	add_action( 'trx_addons_action_importer_import_end', 'trx_addons_cpt_cars_fix_option_names' );
	/**
	 * Fix a keys 'cars_fuel', 'cars_transmission', 'cars_type_of_drive' in the options array
	 * to avoid problems with the names of parameters in the shortcodes and widgets.
	 *
	 ...
	 */
	function trx_addons_cpt_cars_fix_option_names( $new_version = '', $old_version = '' ) {
		if ( empty( $old_version ) ) {
			$old_version = get_option( 'trx_addons_version', '1.0' );
		}
		if ( version_compare( $old_version, '2.34.7.1', '<' ) || current_action() == 'trx_addons_action_importer_import_end' ) {
			$options = apply_filters( 'trx_addons_filter_load_options', get_option( 'trx_addons_options' ) );
			$modified = false;
			foreach( $options as $k => $v ) {
				if ( in_array( $k, array( 'cars_fuel', 'cars_transmission', 'cars_type_of_drive' ) ) ) {
					$options[ $k . '_list' ] = $v;
					unset( $options[ $k ] );
					$modified = true;
				}
	...
	function trx_addons_cpt_cars_fill_custom_column( $column_name = '', $post_id = 0 ) {
		...
		if ( $column_name == 'cpt_cars_image' ) {
			...
			if ( $meta['agent_type'] == 'owner' && ! empty( $meta['owner_name'] ) ) {
				?><div class="trx_addons_meta_row">
					<span class="trx_addons_meta_label"><?php esc_html_e('Owner/Seller', 'trx_addons'); ?></span>
