	...
	function trx_addons_cpt_setup() {
		...
		$TRX_ADDONS_STORAGE['cpt_list'] = apply_filters('trx_addons_cpt_list', array(
			...
			'agents' => array(
				...
				'parent' => 'properties',	// Parent CPT slug
...
if (!function_exists('trx_addons_cpt_load')) {
	...
	function trx_addons_cpt_load() {
		static $loaded = false;
		if ( $loaded || ! trx_addons_is_compatible_theme() ) return;
		$loaded = true;
		global $TRX_ADDONS_STORAGE;
		if (is_array($TRX_ADDONS_STORAGE['cpt_list']) && count($TRX_ADDONS_STORAGE['cpt_list']) > 0) {
			foreach ($TRX_ADDONS_STORAGE['cpt_list'] as $cpt=>$params) {
...
if (!function_exists('trx_addons_cpt_layouts_get_list_options')) {
	function trx_addons_cpt_layouts_get_list_options($add_parameters=array()) {
		return apply_filters('trx_addons_cpt_list_options', array_merge( array(
				// Layouts settings
				...
				'layouts_info' => array(
					"title" => esc_html__('Theme Elements', 'trx_addons') . trx_addons_get_theme_doc_link( '#themerex_addons_theme_elements' ),
				...
				'layouts_create' => array(
					...
					"desc" => wp_kses_data( __('Press the button above to add/restore a set of layouts, created for this theme. These layouts have been automatically added after the theme activation. Use this button to restore deleted layouts. If a layout with the same name already exists, it will be skipped.', 'trx_addons') ),
					"std" => 'trx_addons_cpt_layouts_create',
					"type" => "button"
				)
			),
			! trx_addons_exists_elementor_pro() ? array() : array(
				'allow_elementor_pro_header_footer' => array(
					"title" => esc_html__('Allow Elementor Pro headers & footers', 'trx_addons'),
					"desc" => wp_kses_data( __('Allow Elementor Pro to replace headers and footers from a theme.', 'trx_addons') ),
					"std" => 0,
					"type" => "switch"
				)
			)
...
if (!function_exists('trx_addons_cpt_permalinks_options_fields')) {
	function trx_addons_cpt_permalinks_options_fields( $list ) {
		foreach( $list as $cpt => $params ) {
			...
			if (   ! empty( $params['preloaded'] )
				|| ( empty( $params['parent'] ) && ! trx_addons_components_is_allowed( 'cpt', $cpt ) )
				|| ( ! empty( $params['parent'] ) && ! trx_addons_components_is_allowed( 'cpt', $params['parent'] ) )
				|| ! trx_addons_is_off( trx_addons_get_option( $cpt . '_disable', 0, false ) )
			) {
				continue;
			}
