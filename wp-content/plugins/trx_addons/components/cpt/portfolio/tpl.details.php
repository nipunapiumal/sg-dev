...
if (!empty($meta['details']) && count($meta['details']) > 0 && !empty($meta['details'][0]['title'])) {
	?><div class="sc_portfolio_details <?php echo esc_attr( $class ); ?>"><?php
		$count = 0;
		...
		if ( ! empty( $meta['button_link'] ) && ( ! isset( $meta['button_link_hide_in_archive'] ) || (int)$meta['button_link_hide_in_archive'] == 0 || is_single() ) ) {
