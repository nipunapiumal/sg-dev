...
if ( ! empty( $args['parent_post'] ) && ! trx_addons_is_off( $args['parent_post'] ) ) {
	$parent_post_meta_key = apply_filters( 'trx_addons_filter_parent_post_meta_key', '', $args['post_type'] );
	if ( ! empty( $parent_post_meta_key ) ) {
		$query_args = trx_addons_query_add_meta( $query_args, $parent_post_meta_key, $args['parent_post'] );
	}
}
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		while ( $query->have_posts() ) { $query->the_post();
			trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_CPT . 'team/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $args['type'] ) ) . '-item.php',
