		...
		if ( is_array( $menu_items ) ) {
			foreach ( $menu_items as $k => $item ) {
				if ( ! empty( $item->settings['layout_submenu'] ) && (int)$item->settings['layout_submenu'] > 0 ) {
					if ( ! in_array( 'menu-item-has-children', $menu_items[$k]->classes ) ) {
						$menu_items[$k]->classes[] = 'menu-item-has-children';
					}
					if ( ! in_array( 'menu-item-has-children-layout', $menu_items[$k]->classes ) ) {
						$menu_items[$k]->classes[] = 'menu-item-has-children-layout';
					}
					if ( ! empty( $args->menu_class ) && strpos( $args->menu_class, 'trx-addons-nav-menu' ) !== false && ! in_array( 'trx-addons-mega-nav-item', $menu_items[$k]->classes ) ) {
				...
				if ( empty( $args->menu_class ) || strpos( $args->menu_class, 'trx-addons-nav-menu' ) === false ) {
					if ( ! empty( $item->settings['submenu_pos'] ) && $item->settings['submenu_pos'] == 'static' && ! in_array( 'menu-item-position-static', $menu_items[$k]->classes ) ) {
						$menu_items[$k]->classes[] = 'menu-item-position-static';
					}
					if ( ! empty( $item->settings['badge_text'] ) && ! in_array( 'menu-item-has-badge', $menu_items[$k]->classes ) ) {
						$menu_items[$k]->classes[] = 'menu-item-has-badge';
					}
					if ( ! empty( $item->settings['icon'] ) && ! in_array( 'menu-item-has-icon', $menu_items[$k]->classes ) ) {
						$menu_items[$k]->classes[] = 'menu-item-has-icon';
					}
				}
				if ( ! empty( $item->settings['submenu_fullwidth'] ) && ! in_array( 'trx_addons_' . $item->settings['submenu_fullwidth'], $menu_items[$k]->classes ) ) {
		...
		return apply_filters( 'trx_addons_filter_menu_item_settings', array(
			'layout_submenu' => array(
				...
				'desc'  => wp_kses(
								...
								. ' '
								. sprintf( '<a href="%1$s" class="trx_addons_post_editor trx_addons_hidden"' . trx_addons_external_links_target( true ) . '>%2$s</a>',
			...
			'submenu_pos' => array(
				...
				'desc'  => wp_kses( __( 'A position of the submenu.', 'trx_addons' ), 'trx_addons_kses_content' ),
				'dependency' => array(
					'layout_submenu' => array( '^none' ),
					'submenu_fullwidth' => array( 'is_empty', 'no_stretch' ),
				),
				...
				'options' => array(
					...
					'static'  => esc_html__( 'Centered', 'trx_addons' ),
				),
				'type'    => 'select',
			),
			'icon' => array(
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( is_array( $this->layouts ) && count( $this->layouts ) > 1 ) {
				$html = '<p class="field-layout-submenu description description-wide">
						<label for="edit-menu-item-layout-submenu-' . esc_attr( $item_id ) . '">'
							...
							. sprintf( '<a href="%1$s" class="trx_addons_post_editor' . ( empty( $item->layout_submenu ) || intval( $item->layout_submenu ) == 0 ? ' trx_addons_hidden' : '' ) . '"' . trx_addons_external_links_target( true ) . '>%2$s</a>',
			...
			*/
			...
			$item_html = preg_replace(
				...
				'$1<a class="item-settings" id="settings-' . $item_id . '" href="#" role="button">' . __( 'Settings', 'trx_addons' ) . '</a>'
