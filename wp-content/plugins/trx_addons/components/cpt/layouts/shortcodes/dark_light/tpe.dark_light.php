...
for ( var bp in breakpoints ) {
	...
	if ( settings['position' + suffix] !== '' || position === '' ) {
		...
		bp_css += 'position:' + ( position == 'static' ? 'relative' : 'fixed' ) + ';'
					+ 'z-index:' + ( position == 'static' ? '1' : '999999' ) + ';';
	}
	if ( position != 'static' ) {
		was_non_static = true;
...
#><a href="#" role="button" class="sc_layouts_dark_light sc_layouts_dark_light_{{ settings.type }} sc_layouts_dark_light_effect_{{ settings.effect }} sc_layouts_dark_light_active_{{ active }} {{ className }}<?php
