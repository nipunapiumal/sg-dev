	...
	function trx_addons_add_search_form( $args = array() ) {
		$args = array_merge(
					array(
						...
						'icon_hidden'   => '',
						'search_opened' => '',
						'placeholder_text' => __( 'Search', 'trx_addons' ),
						'close_label_text' => '',
						'overlay_animation' => '',
						'overlay_animation_exit' => '',
						'overlay_animation_duration' => '',
					),
					is_array( $args ) ? $args : array( 'style' => (string)$args )
				);
	...
	function trx_addons_callback_ajax_search() {
		...
		if ( ! empty( $s ) ) {
			...
			if (empty($response['data'])) {
				...
				$response['data'] .= '<article class="post_item"><a href="#" role="button" class="post_more search_more">' . esc_html__('More results ...', 'trx_addons') . '</a></article>';
			}
		} else {
...
if ( !function_exists( 'trx_addons_sc_layouts_search' ) ) {
	function trx_addons_sc_layouts_search($atts, $content = ''){	
		...
		trx_addons_get_template_part(array(
										TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'search/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
