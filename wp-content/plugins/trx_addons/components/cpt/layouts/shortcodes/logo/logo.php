...
if ( !function_exists( 'trx_addons_sc_layouts_logo' ) ) {
	function trx_addons_sc_layouts_logo($atts, $content = ''){	
		...
		if ( ( $customizable && $atts['logo_type'] !== 'custom' ) || ( ! $customizable && empty( $atts['logo'] ) ) ) {
			$logo = apply_filters('trx_addons_filter_theme_logo', '', $atts['logo_type'], 'sc_layouts_logo');
			if ( is_array( $logo ) ) {
		...
		trx_addons_get_template_part(array(
										TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'logo/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
