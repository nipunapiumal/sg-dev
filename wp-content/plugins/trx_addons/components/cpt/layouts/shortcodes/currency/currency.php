...
if ( !function_exists( 'trx_addons_sc_layouts_currency' ) ) {
	function trx_addons_sc_layouts_currency($atts, $content = ''){	
		$atts = trx_addons_sc_prepare_atts( 'trx_sc_layouts_currency', $atts, trx_addons_sc_common_atts( 'trx_sc_layouts_currency', 'id,hide', array(
			// Individual params
			"type" => "default",
		) ) );
		...
		if ( trx_addons_exists_woocommerce() ) {
			...
			trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'currency/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
