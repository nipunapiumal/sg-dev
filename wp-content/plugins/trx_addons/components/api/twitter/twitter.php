	...
	function trx_addons_get_twitter_data_v2( $cfg ) {
		...
		if ( ! empty( $cfg['username'] ) ) {
			...
			if ( ! $data ) {
				...
				$data = trx_addons_unserialize( $data );
			}
		}
		return $data;
	}
		...
		if ( ! empty( $tweet['entities']['urls'] ) || ! empty( $tweet['entities']['media'] ) ) {
			if ( ! empty( $tweet['entities']['urls'] ) && count( $tweet['entities']['urls'] ) > 0 ) {
				foreach ( $tweet['entities']['urls'] as $url ) {
					$text = str_replace( $url['url'], '<a href="' . esc_url( $url['expanded_url'] ) . '"' . trx_addons_external_links_target() . '>' . $url['display_url'] . '</a>', $text );
				}
			}
			...
			$text = preg_replace( '/@([^\s]*)/', '<a href="https://twitter.com/$1"' . trx_addons_external_links_target() . '>@$1</a>', $text );
		}
		return $text;
	}
}
