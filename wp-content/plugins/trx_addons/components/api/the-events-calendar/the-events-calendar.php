		...
		if ( trx_addons_is_tribe_events_page() ) {
			if ( function_exists( 'tribe_get_events_title' ) ) {
				if ( trx_addons_is_single() ) {
					...
					if ( ! empty( $obj->taxonomy ) && in_array( $obj->taxonomy, array( 'post_tag', Tribe__Events__Main::TAXONOMY ) ) ) {
						$title = ucfirst( $obj->name );
					} else {
						$title = strip_tags( tribe_get_events_title( true ) );
					}
				}
			}
		}
		...
		if ( trx_addons_is_tribe_events_page() ) {
			if ( ( $url = get_post_type_archive_link( Tribe__Events__Main::POSTTYPE ) ) != '') {
				...
				if ( is_object( $obj ) && ! empty( $obj->labels->all_items ) ) {
					if ( trx_addons_is_singular( Tribe__Events__Main::POSTTYPE ) ) {
						if ( empty( $link ) ) {
							$link = '<a class="breadcrumbs_item all_posts" href="' . esc_url( $url ) . '">' . esc_html( $obj->labels->all_items ) . '</a>';
						}
					} else {
						$q_obj = get_queried_object();
						if ( ! empty( $q_obj->taxonomy ) && $q_obj->taxonomy == 'post_tag' ) {
							$link = '<a class="breadcrumbs_item all_posts" href="' . esc_url( $url ) . '">' . esc_html( $obj->labels->all_items ) . '</a>';
						}
					}
				}
...
if ( ! function_exists( 'trx_addons_events_get_start_time' ) ) {
	// Commented out for compatibility with a new Tribe Events Calendar GB-blocks
	// add_filter( 'tribe_get_start_time', 'trx_addons_events_get_start_time',10 ,2 );
	/**
	 * Time Format for event start time inside the shortcode "Tribe Events"
	 * 
	 * @hooked tribe_get_start_time
	 * 
	 * @param string $tribe_format_date  Formatted time
	 * @param object $event              Event object
	 * 
	 * @return string                    Formatted time
	 */
	function trx_addons_events_get_start_time( $tribe_format_date, $event ) {
		$start_date = '';
		if ( trx_addons_sc_stack_check( 'trx_sc_events' ) && class_exists( 'Tribe__Events__Timezones' ) ) {
			$start_date = Tribe__Events__Timezones::event_start_timestamp( $event->ID, null );
		}
		return tribe_format_date( $start_date, true, tribe_get_time_format() );
	}
}

if ( ! function_exists( 'trx_addons_events_get_end_time' ) ) {
	// Commented out for compatibility with a new Tribe Events Calendar GB-blocks
	// add_filter( 'tribe_get_end_time', 'trx_addons_events_get_end_time',10 ,2 );
	/**
	 * Time Format for event end time
	 * 
	 * @hooked tribe_get_end_time
	 * 
	 * @param string $tribe_format_date  Formatted time
	 * @param object $event              Event object
	 * 
	 * @return string                    Formatted time
	 */
	function trx_addons_events_get_end_time( $tribe_format_date, $event ) {
		$end_date = '';
		if ( trx_addons_sc_stack_check( 'trx_sc_events' ) && class_exists( 'Tribe__Events__Timezones' ) ) {
			$end_date = Tribe__Events__Timezones::event_end_timestamp( $event->ID, null );
		}
		return tribe_format_date( $end_date, true, tribe_get_time_format() );
	}
}
