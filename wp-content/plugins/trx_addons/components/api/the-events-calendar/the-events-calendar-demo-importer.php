...
if ( ! function_exists( 'trx_addons_tribe_events_importer_export_options' ) ) {
	add_filter( 'trx_addons_filter_export_options', 'trx_addons_tribe_events_importer_export_options', 10, 2 );
	/**
	 * Delete Google API key from the exported options
	 * 
	 ...
	 * @return array            Modified options
	 */
	function trx_addons_tribe_events_importer_export_options( $options, $importer ) {
		if ( trx_addons_exists_tribe_events() && in_array( 'the-events-calendar', $importer->options['required_plugins'] ) ) {
			if ( ! empty( $options['tribe_events_calendar_options']['google_maps_js_api_key'] ) ) {
				$options['tribe_events_calendar_options']['google_maps_js_api_key'] = '';
			}
		}
		return $options;
	}
...
if ( ! function_exists( 'trx_addons_tribe_events_importer_theme_options_data' ) ) {
	add_filter( 'trx_addons_filter_import_theme_options_data', 'trx_addons_tribe_events_importer_theme_options_data', 10, 1 );
	/**
	 * Delete Google API key while the options are imported
	 * 
	 ...
	 * @return array			Modified options
	 */
	function trx_addons_tribe_events_importer_theme_options_data( $data ) {
		if ( ! empty( $data['tribe_events_calendar_options']['google_maps_js_api_key'] ) ) {
			$data['tribe_events_calendar_options']['google_maps_js_api_key'] = '';
		}
		return $data;
	}
}
...
if ( ! function_exists( 'trx_addons_tribe_events_importer_export' ) ) {
	...
	/**
	 * Export a plugin tables to the file
	 * 
	...
	function trx_addons_tribe_events_importer_export( $importer ) {
		if ( trx_addons_exists_tribe_events() && in_array( 'the-events-calendar', $importer->options['required_plugins'] ) ) {
			// Check if events exists in the table 'wp_posts'
			global $wpdb;
			...
			// If no events in 'wp_posts' table - maybe they are moved to the new custom tables - export them.
			// Owerwise, export nothing, because after the demo-data imported, we are regenerate the events cache in the 'tec_events' and 'tec_occurrences' tables
			// in the hook 'trx_addons_action_importer_import_end' below
			if ( ! is_array( $events ) || count( $events ) == 0 ) {
				trx_addons_fpc(
					$importer->export_file_dir( 'the-events-calendar.txt' ),
...
if ( ! function_exists( 'trx_addons_tribe_events_importer_import_end' ) ) {
	...
	/**
	 * Fix for the version TEC 6.0+ to enable posts are created in the previous version
	 * 
