...
if ( ! function_exists( 'trx_addons_wpml_importer_after_import_end' ) ) {
	add_filter( 'trx_addons_action_importer_import_end', 'trx_addons_wpml_importer_after_import_end', 10, 1 );
	/**
	 * Add a translation info to posts and pages after import
	 * 
	...
	function trx_addons_wpml_importer_after_import_end( $importer ) {
		if ( trx_addons_exists_wpml() && in_array( 'sitepress-multilingual-cms', $importer->options['required_plugins'] ) ) {
			...
			if ( ! empty( $iclTranslationManagement ) && is_object( $iclTranslationManagement ) && method_exists( $iclTranslationManagement, 'add_missing_language_information' ) ) {
