	...
	function trx_addons_elm_add_params_fly( $element, $section_id, $args ) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'common', 'common-optimized' ) ) && $section_id == '_section_responsive' ) {
			...
			$element->add_responsive_control( 'fly_scale', array(
													...
													'selectors' => array(
														'{{WRAPPER}} .elementor-widget-container' => '--trx-addons-fly-scale:{{SIZE}}; transform: scale(var(--trx-addons-fly-scale), var(--trx-addons-fly-scale)) rotate(var(--trx-addons-fly-rotate, 0));'
													),
									) );
			...
			$element->add_responsive_control( 'fly_rotate', array(
													...
													'selectors' => array(
														'{{WRAPPER}} .elementor-widget-container' => '--trx-addons-fly-rotate:{{SIZE}}deg; transform: scale(var(--trx-addons-fly-scale,1), var(--trx-addons-fly-scale,1)) rotate(var(--trx-addons-fly-rotate));'
													),
									) );
