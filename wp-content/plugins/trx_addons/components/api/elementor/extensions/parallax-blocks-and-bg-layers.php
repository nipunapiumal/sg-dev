...
if ( ! function_exists( 'trx_addons_elm_add_parallax_blocks' ) ) {
	...
	function trx_addons_elm_add_parallax_blocks( $element, $section_id, $args ) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'container' ) ) && $section_id == 'section_border' ) {	//_section_responsive
			...
			$element->start_controls_section( 'section_trx_parallax', array(
																		'tab' => $tab,
																		'label' => __( 'Background Layers', 'trx_addons' )	// . trx_addons_get_theme_doc_link( '#elementor_extension_bg_layers' ),
			) );
...
if ( ! function_exists( 'trx_addons_elm_add_parallax_blocks_data' ) ) {
	...
	add_action( 'elementor/frontend/container/before_render', 'trx_addons_elm_add_parallax_blocks_data', 10, 1 );
	/**
	...
	function trx_addons_elm_add_parallax_blocks_data( $element ) {
		if ( is_object( $element ) && in_array( $element->get_name(), array( 'section', 'column', 'container' ) ) ) {
...
if ( ! function_exists('trx_addons_elm_add_parallax_params_to_widgets' ) ) {
	...
	function trx_addons_elm_add_parallax_params_to_widgets($element, $section_id, $args) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'container', 'common', 'common-optimized' ) ) && $section_id == '_section_responsive' ) {
			...
			$element->start_controls_section( 'section_trx_entrance', array(
																		'tab' => ! empty( $args['tab'] ) ? $args['tab'] : \Elementor\Controls_Manager::TAB_ADVANCED,
																		'label' => __( 'Animation', 'trx_addons' )	// . trx_addons_get_theme_doc_link( '#elementor_extension_animation' ),
																	) );
			...
			$element->add_responsive_control( 'parallax_range_start', array(
			...
			$element->add_responsive_control( 'parallax_range_end', array(
			...
			$element->add_responsive_control( 'parallax_sticky_offset', array(
			...
			foreach ( array( 'start', 'end' ) as $tab ) {
				...
				$element->add_responsive_control( "parallax_x_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_y_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_opacity_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_blur_{$tab}", array(
														'label' => __( 'Blur', 'trx_addons' ),
														'type' => \Elementor\Controls_Manager::SLIDER,
														'default' => array(
															'size' => 0,
															'unit' => 'px'
				...
				$element->add_responsive_control( "parallax_backdrop_blur_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_scale_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_rotate_{$tab}", array(
				...
				$element->add_responsive_control( "parallax_crop_{$tab}", array(
			...
			$element->add_control( 'parallax_transition', array(
													'label' => __( 'Transition', 'trx_addons' ),
													'type' => \Elementor\Controls_Manager::SWITCHER,
													'default' => '1',
													'return_value' => '1',
													'condition' => array(
			...
			$element->add_responsive_control( 'parallax_lag', array(
...
if ( ! function_exists( 'trx_addons_elm_add_parallax_data_to_widgets' ) ) {
	...
	// After Elementor 3.16.0
	add_action( 'elementor/frontend/container/before_render',  'trx_addons_elm_add_parallax_data_to_widgets', 10, 1 );
	/**
	...
	function trx_addons_elm_add_parallax_data_to_widgets( $element ) {
		...
		if ( ! empty( $parallax ) || ! empty( $parallax_mouse ) ) {
			...
			// Array with a list of general params (not depend from responsive breakpoints)
			$data = array(
				...
				'transition' => ! empty( $parallax ) && ! empty( $settings['parallax_transition'] ) ? 1 : 0,
			...
			$params = array(
				'range_start' => array( 'default' => 0 ),
				'range_end' => array( 'default' => 40 ),
				'sticky_offset' => array( 'default' => 0 ),
				'lag' => array( 'default' => 0 ),
				'x_start' => array( 'default' => 0, 'unit' => 'px' ),
				'x_end' => array( 'default' => 0, 'unit' => 'px' ),
				'y_start' => array( 'default' => 0, 'unit' => 'px' ),
				'y_end' => array( 'default' => 0, 'unit' => 'px' ),
				'scale_start' => array( 'default' => 100 ),
				'scale_end' => array( 'default' => 100 ),
				'rotate_start' => array( 'default' => 0 ),
				'rotate_end' => array( 'default' => 0 ),
				'opacity_start' => array( 'default' => 1 ),
				'opacity_end' => array( 'default' => 1 ),
				'crop_start' => array( 'default' => 0 ),
				'crop_end' => array( 'default' => 100 ),
				'blur_start' => array( 'default' => 0 ),
				'blur_end' => array( 'default' => 0 ),
				'backdrop_blur_start' => array( 'default' => 0 ),
				'backdrop_blur_end' => array( 'default' => 0 ),
			);
			$suffix_prev = '';
			foreach ( $bp as $resp => $max ) {
				...
				foreach ( $params as $param => $default ) {
					...
					if ( ! empty( $suffix ) && ! isset( $settings["parallax_{$param_name}"] ) ) continue;
					// Set parameter value
					$data[ $param_name ] = isset( $settings["parallax_{$param_name}"] ) && $settings["parallax_{$param_name}"]['size'] !== ''
											? $settings["parallax_{$param_name}"]['size']
											: ( empty( $suffix ) || ! isset( $data[ $param_prev ] ) ? $default['default'] : $data[ $param_prev ] );
					if ( isset( $default['unit'] ) ) {
						$data[ "{$param_name}_unit" ] = isset( $settings["parallax_{$param_name}"]['unit'] ) && $settings["parallax_{$param_name}"]['size'] !== ''
											? $settings["parallax_{$param_name}"]['unit']
											: ( empty( $suffix ) || ! isset( $data[ "{$param_prev}_unit" ] ) ? $default['unit'] : $data[ "{$param_prev}_unit" ] );
					}
				}
				// Set equal units for start and end values if one of them is zero
				if ( isset( $data["x_start{$suffix}"] ) && isset( $data["x_end{$suffix}"] ) ) {
				...
				if ( isset( $data["y_start{$suffix}"] ) && isset( $data["y_end{$suffix}"] ) ) {
			...
			$element->add_render_attribute( '_wrapper', 'data-parallax-params', json_encode( $data ) );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_elm_convert_params_parallax' ) ) {
	...
	add_action( 'admin_init', 'trx_addons_elm_convert_params_parallax' );
	/**
	...
	function trx_addons_elm_convert_params_parallax( $new_version = '', $old_version = '' ) {
		$db_key    = 'elm_convert_params_parallax';
		...
		if ( version_compare( $old_version, '2.18.0', '<' ) || current_action() == 'trx_addons_action_importer_import_end' || $db_offset > 0 ) {
			global $wpdb;
...
//					&& ! isset( $elm['settings']['parallax_flow'] )
					...
					// Animate From & To: X, Y, Scale, Rotate, Opacity, Blur, Backdrop Blur
					$atts = array( 'x' => 0, 'y' => 0, 'scale' => 100, 'rotate' => 0, 'opacity' => 1, 'blur' => 0, 'backdrop_blur' => 0 );
