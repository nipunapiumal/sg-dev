...
if ( ! function_exists( 'trx_addons_elm_tab_template_add_layout' ) ) {
	...
	function trx_addons_elm_tab_template_add_layout( $html, $element ) {
		if ( trx_addons_get_option( 'sc_tabs_layouts', 0 ) > 0 && is_object( $element ) ) {
			$el_name = $element->get_name();
			if ( $el_name == 'tabs' ) {
					...
					foreach( $tabs as $k => $tab ) {
						...
						if ( ! empty( $layout ) ) {
							...
							*/
							...
							if ( preg_match(
										// This way is broken a layout, if accordion is present in the layout.
										// Because the accordion has the same structure and class names as the tabs
										// '~(<div[^>]*class="elementor-tab-content[^>]*data-tab="' . ( $k + 1 ) . '"[^>]*>)([\s\S]*)(</div>)~U',
										// This way is more reliable - search the tab's content by ID
										'~(<div[^>]*id="' . ( $render_atts["tabs.{$k}.tab_content"]['id'][0] ) . '"[^>]*data-tab="' . ( $k + 1 ) . '"[^>]*>)([\s\S]*)(</div>)~U',
										$html,
										$matches
									)
							) {
...
if ( ! function_exists( 'trx_addons_elm_modify_tabs' ) ) {
	...
	function trx_addons_elm_modify_tabs( $widgets_manager ) {
		if ( trx_addons_get_option( 'sc_tabs_layouts', 0 ) > 0
