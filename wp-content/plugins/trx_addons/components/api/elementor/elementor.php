...
if ( ! function_exists( 'trx_addons_elm_save_post_from_editor' ) ) {
	...
	function trx_addons_elm_save_post_from_editor() {
		...
		if ( (int)$id > 0 && trx_addons_current_user_can_edit( $id ) ) {
			...
			$resp_params = apply_filters(
				...
				$area == 'sc_slider_wrapper'
					? array(
						...
						'slides_min_width' => 'slides-min-width',
						)
					: array(
...
if ( ! function_exists( 'trx_addons_elm_render_icon' ) ) {
	function trx_addons_elm_render_icon( $icon_settings, $defa = '', $echo = true ) {
		if ( trx_addons_exists_elementor() ) {
			if ( empty( $icon_settings['value'] ) && ! empty( $defa ) ) {
				$icon_settings = [
					'library' => substr( $defa, 0, 6 ) == 'eicon-' ? 'eicons' : ( strpos( $defa, 'far-' ) !== false ? 'fa-regular' : 'fa-solid' ),
					'value' => $defa,
				];
			}
			if ( ! empty( $icon_settings['value'] ) ) {
				$html = \Elementor\Icons_Manager::try_get_icon_html( $icon_settings, [ 'aria-hidden' => 'true' ] );
				if ( empty( $html ) ) {
					$html = '<i class="' . esc_attr( $defa ) . '" aria-hidden="true"></i>';
				}
				if ( $echo ) {
					echo $html;
				} else {
...
require_once TRX_ADDONS_PLUGIN_DIR . TRX_ADDONS_PLUGIN_API . 'elementor/extensions/blur.php';
