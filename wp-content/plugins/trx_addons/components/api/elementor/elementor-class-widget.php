...
if ( class_exists('\Elementor\Widget_Base') && ! class_exists('TRX_Addons_Elementor_Widget') ) {
	...
	abstract class TRX_Addons_Elementor_Widget extends \Elementor\Widget_Base {
		...
		protected $styles_allowed = false;
		...
		public function __construct( $data = [], $args = null ) {
			...
			$this->styles_allowed = apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, str_replace( array( 'trx_sc_', 'trx_widget_'), array( 'sc_', 'widget_' ), $this->get_name() ) );
		}
		...
		public function get_custom_help_url() {
			$substitutes = array(
				'trx_widget_socials' => 'trx_widget_socials_widget',	// To avoid conflict with the 'trx_sc_socials' shortcode
			);
			$sc = $this->get_sc_name();
			if ( isset( $substitutes[ $sc ] ) ) {
				$sc = $substitutes[ $sc ];
			}
			return trx_addons_get_theme_doc_url( '#shortcodes_settings_' . str_replace( array( 'trx_sc_', 'trx_widget_', 'trx_sc_widget_' ), '', $sc ) );
		}
		...
		/**
		 * Add navigation arrows controls to the slider
		 * 
		 * @param array $args  Additional parameters to add or change some default parameters. 
		 * 						Can contain two keys: 'prev' => array() and 'next' => array()
		 * 
		 * @access protected
		 */
		protected function add_slider_arrows( $prefix = '', $args = array() ) {
			...
			$this->add_control(
				"{$prefix}_prev", array_merge(
				[
					'label' => esc_html__( 'Previous Arrow Icon', 'trx_addons' ),
				...
				! empty( $args['prev'] ) && is_array( $args['prev'] ) ? $args['prev'] : array()
			) );

			$this->add_control(
				"{$prefix}_next", array_merge(
				[
					'label' => esc_html__( 'Next Arrow Icon', 'trx_addons' ),
					'type' => \Elementor\Controls_Manager::ICONS,
					// 'fa4compatibility' => 'icon',
					'skin' => 'inline',
					'label_block' => false,
				...
				! empty( $args['next'] ) && is_array( $args['next'] ) ? $args['next'] : array()
			) );
		}
...
//					),
		...
		protected function get_query_param( $pt ) {
			...
			$params = array(
				trx_addons_get_option( 'sc_ids_type', 'advanced' ) == 'simple'
					? array(
							'name' => 'ids',
							'type' => \Elementor\Controls_Manager::TEXT,
								...
								if ( ! empty( $p ) && ( ! empty( $value ) || $i === 0 ) ) {
									...
									if ( isset( $v[ $parts[0] ] ) ) {
										$value .= ! empty( $parts[1] )
													...
													: ( is_array( $v[ $parts[0] ] )	 // If the value is array - get the key 'url' or a first value if 'url' is not exists. Used for svg-icons
														? ( isset( $v[ $parts[0] ]['url'] )
															? $v[ $parts[0] ]['url']
															: trx_addons_array_get_first_value( $v[ $parts[0] ] )
															)
														: $v[ $parts[0] ]
														);
									}
