...
if ( ! function_exists( 'trx_addons_edd_load_scripts_admin' ) ) {
	add_action("trx_addons_action_load_scripts_admin", 'trx_addons_edd_load_scripts_admin');
	/**
	 ...
	 */
	function trx_addons_edd_load_scripts_admin( $all = false ) {
		if ( ! trx_addons_exists_edd() || trx_addons_get_value_gp( 'post_type' ) != TRX_ADDONS_EDD_PT ) {
			return;
		}
		//wp_enqueue_style(  'trx_addons-edd-admin',  trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'easy-digital-downloads/easy-digital-downloads-admin.css' ), array(), null );
		wp_enqueue_script( 'trx_addons-edd-admin',  trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'easy-digital-downloads/easy-digital-downloads-admin.js' ), array('jquery'), null, true );
	}
}
		...
		?><div class="trx_addons_buttons trx_addons_edd_purchase_buttons"><?php
			...
			if ( ! empty( $trx_addons_meta['download_url'] ) ) {
				?><div class="edd_download_purchase_form"><?php
					?><a href="<?php echo esc_url( trx_addons_add_referals_to_url( $trx_addons_meta['download_url'], trx_addons_get_option( 'edd_referals' ) ) ); ?>" class="sc_button"<?php echo trx_addons_external_links_target(); ?>><?php
						echo true == $free
...
if ( ! function_exists( 'trx_addons_edd_add_demo_button' ) ) {
	...
	function trx_addons_edd_add_demo_button( $post_id = 0, $args = array(), $trx_addons_meta = false ) {
		...
		if ( ! empty( $url ) ) {
			?><a href="<?php echo esc_url( $url ); ?>" class="sc_button"<?php if ( ! $details ) echo trx_addons_external_links_target(); ?>><?php
				if ( $details ) {
...
if ( ! function_exists( 'trx_addons_edd_cart_show' ) ) {
	add_filter( 'trx_addons_filter_sc_layouts_cart_show', 'trx_addons_edd_cart_show', 10, 2 );
	/**
	 * Display cart if a parameter 'market' is 'edd' and EDD plugin is active
	 * 
	 * @hooked trx_addons_filter_sc_layouts_cart_show
	 *
	 * @param bool  $show  Current state of the cart
	 * @param array $atts  Shortcode attributes
	 * 
	 * @return bool        New state of the cart
	 */
	function trx_addons_edd_cart_show( $show, $atts ) {
		if ( ! empty( $atts['market'] ) && $atts['market'] == 'edd' ) {
			$show = trx_addons_exists_edd();
		}
		return $show;
	}
}

if ( ! function_exists( 'trx_addons_edd_sc_layouts_cart_enabled' ) ) {
	add_filter( 'trx_addons_filter_sc_layouts_cart_enabled', 'trx_addons_edd_sc_layouts_cart_enabled' );
	/**
	 * Enable cart if EDD plugin is active
	 * 
	 * @hooked trx_addons_filter_sc_layouts_cart_enabled
	 *
	 * @param bool  $active  Current state of the cart
	 * 
	 * @return bool        New state of the cart
	 */
	function trx_addons_edd_sc_layouts_cart_enabled( $enabled ) {
		return $enabled || trx_addons_exists_edd();
	}
}
