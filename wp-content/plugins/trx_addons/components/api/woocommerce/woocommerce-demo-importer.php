	...
	function trx_addons_woocommerce_importer_check_options( $allow, $k, $v, $options ) {
		if ( $allow && ( strpos( $k, 'woocommerce_' ) === 0 || strpos( $k, 'shop_' ) === 0 ) ) {
			$allow = trx_addons_exists_woocommerce()
						...
						&& ! in_array( $k, array( 'woocommerce_queue_flush_rewrite_rules', 'woocommerce_coming_soon' ) );
		}
		return $allow;
	}
}
...
if ( ! function_exists( 'trx_addons_woocommerce_importer_after_import_end_clear_wc_cache' ) ) {
	add_filter( 'trx_addons_action_importer_import_end', 'trx_addons_woocommerce_importer_after_import_end_clear_wc_cache', 10, 1 );
	/**
	 * Clear WooCommerce cache after import data is complete
	 * 
	 * @hooked trx_addons_action_importer_import_end
	 *
	 * @param object $importer  Importer object
	 */
	function trx_addons_woocommerce_importer_after_import_end_clear_wc_cache( $importer ) {
		global $wpdb;
		if ( trx_addons_exists_woocommerce() && in_array( 'woocommerce', $importer->options['required_plugins'] ) ) {
			$res = $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_wc_%' OR option_name LIKE '_transient_timeout_wc_%'");
		}
	}
}
