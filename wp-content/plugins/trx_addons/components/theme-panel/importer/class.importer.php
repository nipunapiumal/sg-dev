...
class trx_addons_demo_data_importer {
	...
	// Set transient with a current theme slug as a suffix
	function set_transient( $name, $value, $expire = 20*60 ) {
		set_transient( $name . '_' . $this->theme_slug . '_' . $this->options['demo_type'], $value, $expire );
	}

	// Get transient with a current theme slug as a suffix
	function get_transient( $name ) {
		return get_transient( $name . '_' . $this->theme_slug . '_' . $this->options['demo_type'] );
	}

	// Delete transient with a current theme slug as a suffix
	function delete_transient( $name ) {
		delete_transient( $name . '_' . $this->theme_slug . '_' . $this->options['demo_type'] );
	}
	...
	function show_importer($tab_id, $theme_info) {
		?><div class="trx_importer_section">
			...
			// Importer form
			?><form id="trx_importer_form"><?php

				// Demo type selector (if more than 1)
				...
				// Demo set selector (Partial or Full)
				?>
				...
				<div class="trx_importer_advanced_settings"><?php
					...
					// List of pages (for partial import), options, media
					?><div class="trx_importer_advanced_settings_block trx_importer_advanced_settings_pages"><?php

						// Skins selector (if more than 1)
						$skins = trx_addons_call_theme_function( 'storage_get', array( 'skins' ) );
						if ( is_array( $skins ) && count( $skins ) > 1 ) {
							ksort( $skins );
							?>
							<p class="trx_importer_subtitle"><span data-set-full="1" data-set-part="0"><?php esc_html_e( 'Select the elements to be imported:', 'trx_addons' ); ?></span></p>
							<p class="trx_importer_subtitle trx_importer_subtitle_skins_wizard"><span data-set-full="0" data-set-part="1"><?php esc_html_e( 'Select the elements to be imported:', 'trx_addons' ); ?></span></p>
							<p class="trx_importer_subtitle trx_importer_subtitle_skins_no_wizard"><span data-set-full="0" data-set-part="1"><?php esc_html_e( 'Select the elements to be imported from skin:', 'trx_addons' ); ?></span></p>
							<p class="trx_importer_skins">
								<select size="1" class="trx_importer_skin" value="" name="import_skin" id="import_skin" data-set-full="0" data-set-part="1">
									<option value="" selected><?php esc_html_e( '- Active skin -', 'trx_addons' ); ?></option>
									<?php
									$list = trx_addons_array_from_list( $skins );
									foreach ( $list as $k => $v ) {
										?>
							...
							?><p class="trx_importer_subtitle"><?php esc_html_e('Select the elements to be imported:', 'trx_addons'); ?></p><?php
						}
						...
						<div class="trx_importer_part_pages<?php if ($this->options['demo_set']=='full') echo ' trx_importer_hidden"'; ?>">
							<?php
								$pages = $this->get_list_pages_from_demo();
								if (is_array($pages)) {
						...
						<div class="trx_importer_params">
							...
							$this->show_importer_params(array(
								...
								'part' => 0, 		//1,
								'checked' => true	//$this->options['demo_set']=='full'
							));
							$this->show_importer_params(array(
								'slug' => 'widgets',
					...
					// Select plugins to import its settings, posts (if need) and data
					?><div class="trx_importer_advanced_settings_block trx_importer_advanced_settings_plugins trx_addons_hidden">
			...
			<div id="trx_importer_progress" class="style_<?php echo esc_attr($this->options['demo_style']); ?>">
				...
				<h4 class="trx_importer_progress_result">
					...
					// if (empty($tab_id)) {
						?>
						<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="trx_importer_view_site"<?php echo trx_addons_external_links_target( true ); ?>><?php esc_html_e('View site', 'trx_addons'); ?></a>
						<?php
					// }
					?>
				</h4>
			...
			foreach ($this->options['banners'] as $banner) {
				...
				if (!empty($banner['link_url'])) {
					$banners .= '<a class="trx_banners_item_link' . (empty($banner['link_caption'])	? ' trx_banners_item_link_block' : ' trx_addons_button trx_addons_button_accent') . '"'
									. ' href="' . esc_url($banner['link_url']) . '"' . trx_addons_external_links_target( true ) . '>'
	...
	function importer() {
		...
		if ( $this->action == 'import_start' ) {
			...
			if ( ! $this->check_required_plugins( $this->options['demo_parts'] ) ) {
				...
				if ( $this->options['demo_set']=='full' && ! empty( $_POST['demo_steps'] ) && strpos( $_POST['demo_steps'], 'import_uploads' ) !== false ) {
					trx_addons_clear_folder( $this->uploads_dir );
				}
			}
	...
	function import_uploads($uploads = '', $import_type = '') {
		...
		if ($uploads == '') 
			$uploads = $this->get_file_with( 'uploads' );
		if (empty($uploads))
			return;
		...
		$txt = !$this->options['debug'] ? $this->get_transient('trx_addons_importer_uploads') : '';
		if ( empty($last_arh) || empty($txt) ) {
			if ( ($txt = $this->get_file($uploads, 0, $import_type)) === false) {
				return;
			} else if (!$this->options['debug']) {
				$this->set_transient('trx_addons_importer_uploads', $txt);
			}
		}
		...
		if ($result >= 100) $this->delete_transient('trx_addons_importer_uploads');
	...
	function import_thumbnails() {
		...
		$files = ! $this->options['debug'] ? $this->get_transient('trx_addons_importer_attachments') : '';
		if ( empty($last_arh) || empty($files) ) {
			...
			if ( ! $this->options['debug']) {
				$this->set_transient('trx_addons_importer_attachments', $list);
			}
		}
		// Process files
		...
		if ($result >= 100) $this->delete_transient('trx_addons_importer_attachments');
		$this->response['result'] = $result;
	}
	...
	function import_theme_mods() {
		...
		$file_name = $this->get_file_with( 'mods' );
		if ( empty( $file_name ) )
			return;
		if ( ( $txt = $this->get_file( $file_name ) ) === false )
			return;
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl( sprintf( __( 'Unable to unserialize data from the file %s', 'trx_addons' ), trx_addons_get_file_name( $file_name ) ) );
		}
	}
	...
	function import_theme_options() {
		...
		$file_name = $this->get_file_with( 'options' );
		if ( empty( $file_name ) ) {
			return;
		}
		if ( ( $txt = $this->get_file( $file_name ) ) === false ) {
	...
	function import_widgets() {
		...
		$file_name = $this->get_file_with( 'widgets' );
		if ( empty( $file_name ) ) {
			return;
		}
		if ( ( $txt = $this->get_file( $file_name ) ) === false ) {
	...
	function import_dump( $slug, $title, $required = true ) {
		$file_name = $this->get_file_with( $slug );
		if ($this->options['debug']) {
			dfl( sprintf( __( 'Import dump file: "%s"', 'trx_addons' ), $file_name ) );
		}
		if ( empty( $file_name ) ) {
			return;
		}
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				...
				if ($this->options['demo_set']=='part' && $table=='posts' && $this->response['start_from_id'] > 0) {
					$this->part_replace = $this->get_transient('trx_addons_importer_part_replace');
					if (!is_array($this->part_replace)) $this->part_replace = array();
					$this->part_image = $this->get_transient('trx_addons_importer_part_image');
				...
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					foreach ($rows as $row) {
						...
						if (is_array($row) && count($row) > 0) {
							...
							if ($this->options['demo_set']=='part') {
								if ( $table=='posts' ) {
									// Add a skin slug to the post_title and post_name if the post from another skin
									if ( ! isset( $this->options['files'][ $this->options['demo_type'] ] ) ) {
										$row['post_name']  .= '-' . esc_sql( $this->options['demo_type'] );
										$row['post_title'] .= ' ' . sprintf( __( '(from skin %s)', 'trx_addons' ), $this->options['demo_type'] );
									}
				...
				if ($this->options['demo_set']=='part' && $table=='posts') {
					$this->set_transient('trx_addons_importer_part_replace',	$result < 100 ? $this->part_replace : array(), 10*60);
					$this->set_transient('trx_addons_importer_part_image',		$result < 100 ? $this->part_image 	: array(), 10*60);
				}
	...
	function import_prepare_no_image() {
		...
		if ( empty( $no_image_post->ID ) ) {
			if ( ( $img = trx_addons_get_no_image( '', false, false ) ) != '' ) {
	...
	// Return a name of the file with posts from the selected demo
	function get_file_with( $slug ) {
		$demo_type = $this->options['demo_type'];
		$file_name = '';
		if ( ! empty( $this->options['files'][ $demo_type ]['file_with_' . $slug ] ) ) {
			$file_name = $this->options['files'][ $demo_type ]['file_with_' . $slug ];
		} else {
			$first_key = trx_addons_array_get_first_key( $this->options['files'] );
			if ( isset( $this->options['files'][ $first_key ]['file_with_' . $slug ] ) ) {
				$file_name = $this->options['files'][ $first_key ]['file_with_' . $slug ];
			}
		}
		return $file_name;
	}
	...
	function get_list_pages_from_demo() {
		$list = $this->get_transient( "trx_addons_installer_posts" );
		if ( ! is_array( $list ) ) {
			$list = array();
			$txt = $this->get_file( $this->get_file_with( 'posts' ) );
			if ( ! empty( $txt ) && is_serialized( $txt ) ) {
				$data = trx_addons_unserialize( $txt );
			...
			$this->set_transient( "trx_addons_installer_posts", $list, 5 * 60 );	// Store to cache for 5 minutes
		}
		return $list;
	}
	...
	function get_list_pages_callback() {
		...
		$response = array(
			'error' => ! isset( $_POST['demo_type'] ) ? esc_html__('Incorrect parameters', 'trx_addons') : '',
		);

		if ( isset( $_POST['demo_type'] ) ) {
			// Convert associative array 'id' => 'title' to indexed list of arrays with keys 'id' and 'title' to prevent resorting array by ids in JavaScript
			$response['data'] = trx_addons_list_from_array( $this->get_list_pages_from_demo() );
		}
	...
	function prepare_data( $str ) {
		...
		} else if ( is_object( $str ) && ! is_a( $str, '__PHP_Incomplete_Class' ) ) {
			foreach ( $str as $k => $v ) {
				$str->{$k} = $this->prepare_data( $v );
			}
