...
if ( ! function_exists( 'trx_addons_lazy_load_options' ) ) {
	...
	function trx_addons_lazy_load_options( $options ) {
		...
		if ( trx_addons_lazy_load_allowed() ) {
			trx_addons_array_insert_before($options, 'performance_section_cache', array(
				'performance_section_lazy' => array(
					"title" => esc_html__('Lazy Loading', 'trx_addons'),
					'icon' => 'trx_addons_icon-lodging',
					"type" => "section"
				),
				'lazy_load_info'   => array(
					'title' => esc_html__( 'Lazy Loading', 'trx_addons' ) . trx_addons_get_theme_doc_link( '#themerex_addons_lazy_loading' ),
					'desc'  => '',
					'demo'  => false,
					'type'  => 'info',
				),
...
if ( ! function_exists( 'trx_addons_lazy_load_process_products_images' ) ) {
	...
	function trx_addons_lazy_load_process_products_images( $html = '', $product = null, $size = null, $attr = null, $placeholder = null, $image = null ) {
		if ( ! empty( $html ) && trx_addons_lazy_load_enable() ) {
...
if ( ! function_exists( 'trx_addons_lazy_load_content_process_images' ) ) {
	...
	function trx_addons_lazy_load_content_process_images( $content = '', $preload = true, $img = true, $img2 = true, $bg_img = true, $media = true ) {
		if ( ! empty( $content ) && trx_addons_lazy_load_enable() && ! is_admin() ) {
