...
if ( !function_exists( 'trx_addons_sc_skills' ) ) {
	function trx_addons_sc_skills($atts, $content = ''){	
		...
		if (is_array($atts['values']) && count($atts['values']) > 0) {
			...
			if ( empty( $atts['max'] ) || abs( (float)$atts['max'] ) < 0.0001 ) {
				$atts['max'] = max( 0.0001, $max );
			} else {
			...
			trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_SHORTCODES . 'skills/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
