...
// Add an option 'popup' to the featured image parameters if a 'Blogger' parameter is set to 'Video in popup'
if ( ! function_exists( 'trx_addons_sc_blogger_featured_image_params' ) ) {
	add_filter( 'trx_addons_filter_args_featured', 'trx_addons_sc_blogger_featured_image_params', 10, 3 );
	function trx_addons_sc_blogger_featured_image_params( $params, $sc = '', $args = array() ) {
		if ( ! empty( $args['video_in_popup'] ) ) {
			$params['popup'] = true;
		}
		return $params;
	}
...
if ( ! function_exists( 'trx_addons_sc_blogger' ) ) {
	function trx_addons_sc_blogger($atts, $content = '') {	
		...
		trx_addons_get_template_part(
			array(
				TRX_ADDONS_PLUGIN_SHORTCODES . 'blogger/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
