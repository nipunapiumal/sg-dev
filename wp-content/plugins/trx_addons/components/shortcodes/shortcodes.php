...
*/
			...
			'switcher' => array(
							...
							'layouts_sc' => array(
								'default' => esc_html__('Switcher - Label Out', 'trx_addons'),
								'modern'  => esc_html__('Switcher - Label In', 'trx_addons'),
...
if ( ! function_exists( 'trx_addons_sc_show_attributes' ) ) {
	function trx_addons_sc_show_attributes( $sc, $args, $area, $echo = true ) {
		if ( ! $echo ) {
			ob_start();
		}
...
if ( !function_exists( 'trx_addons_ajax_sc_pagination' ) ) {
	...
	function trx_addons_ajax_sc_pagination() {
		...
		$params = array();
		if ( ! empty( $_POST['params'] ) && is_string( $_POST['params'] ) ) {
			$params = substr( $_POST['params'], 0, 2 ) == 'a:'
						// Old format (params are serialized - if an old template 'templates/tpl.sc_pagination.php' copied to the theme/skin).
						// The second argument is true to disable unserialization if the string is containing objects
						? trx_addons_unserialize( wp_unslash( $_POST['params'] ), true )
						// New format (params are encoded to JSON)
						: json_decode( wp_unslash( $_POST['params'] ), true );
		}
		if ( ! is_array( $params ) ) {
			$params = array();
		}
