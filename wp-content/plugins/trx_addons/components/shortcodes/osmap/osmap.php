...
if ( !function_exists( 'trx_addons_sc_osmap_localize_script' ) ) {
	...
	function trx_addons_sc_osmap_localize_script($vars) {
		if ( trx_addons_components_is_allowed('sc', 'osmap') ) {
			...
			$vars['osmap_attribution'] = wp_kses_data( sprintf( __( 'Map data &copy; %s contributors', 'trx_addons' ), trx_addons_sc_osmap_get_site_link() ) );
		}
		return $vars;
	}
}
...
// Return a link to the OSMap site
if ( !function_exists( 'trx_addons_sc_osmap_get_site_link' ) ) {
	function trx_addons_sc_osmap_get_site_link() {
		return '<a href="https://www.openstreetmap.org/">' . __( 'OpenStreetMap', 'trx_addons' ) . '</a>';
	}
}
		...
		if ( (is_array($atts['markers']) && count($atts['markers']) > 0) || !empty($atts['address'])) {
			...
			if ( empty( $atts['style'] ) ) {
				...
				trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
