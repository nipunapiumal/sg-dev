			...
			if ( settings.before_text || settings.after_text ) {
				#><div class="sc_icompare_overlay"><#
					if ( settings.before_text ) {
						var beforeHTMLTag = elementor.helpers.validateHTMLTag( settings.before_html_tag );
						#><{{{ beforeHTMLTag }}} class="sc_icompare_text_before sc_icompare_text_pos_{{ settings.before_pos }}">{{ settings.before_text }}</{{{ beforeHTMLTag }}}><#
					}
					if ( settings.after_text ) {
						var afterHTMLTag = elementor.helpers.validateHTMLTag( settings.after_html_tag );
						#><{{{ afterHTMLTag }}} class="sc_icompare_text_after sc_icompare_text_pos_{{ settings.after_pos }}">{{ settings.after_text }}</{{{ afterHTMLTag }}}><#
					}
				#></div><#
			}

			#><div class="sc_icompare_handler sc_icompare_handler_style_{{ settings.handler }}<#
				print( ' sc_icompare_handler_' + ( settings.handler_show ? 'show' : 'hide' ) );
				print( ' sc_icompare_separator_' + ( settings.handler_separator ? 'show' : 'hide' ) );
				#>"
				data-handler-pos="{{ settings.handler_pos.size }}"
			><#
				// Separator
				...
				if ( settings.handler_show ) {
					...
					// Handler icon
						...
						if ( icon && icon != 'none' ) {
							...
							shown = true;
						} else if ( settings.custom_icon && settings.custom_icon.value != '' ) {
							var custom_icon_HTML = elementor.helpers.renderIcon( view, settings.custom_icon, { 'aria-hidden': true } );
							if ( custom_icon_HTML ) {
								if ( typeof custom_icon_HTML == 'object' && custom_icon_HTML.value ) {
									#><span class="sc_icompare_handler_icon sc_icompare_handler_custom_icon sc_icon_type_svg">{{{ custom_icon_HTML.value }}}</span><#
								} else if ( typeof custom_icon_HTML == 'string' && custom_icon_HTML.indexOf( '<svg ' ) >= 0 ) {
									#><span class="sc_icompare_handler_icon sc_icompare_handler_custom_icon sc_icon_type_svg">{{{ custom_icon_HTML }}}</span><#
								} else if ( typeof custom_icon_HTML == 'string' && custom_icon_HTML.indexOf( '<i ' ) >= 0 ) {
									#><span class="sc_icompare_handler_icon sc_icompare_handler_custom_icon sc_icon_type_icon">{{{ custom_icon_HTML }}}</span><#
								}
								shown = true;
							}
						}
