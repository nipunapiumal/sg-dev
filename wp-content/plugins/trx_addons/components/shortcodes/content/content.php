...
if ( !function_exists( 'trx_addons_sc_content' ) ) {
	function trx_addons_sc_content($atts, $content=''){	
		$atts = trx_addons_sc_prepare_atts( 'trx_sc_content', $atts, trx_addons_sc_common_atts( 'trx_sc_content', 'id,title', array(
			// Individual params
			'type' => 'default',
			"width" => "",
			"size" => "none",
		...
		if (!empty($atts['content']) || !empty($atts['title']) || !empty($atts['subtitle']) || !empty($atts['description'])) {
			...
			trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_SHORTCODES . 'content/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
