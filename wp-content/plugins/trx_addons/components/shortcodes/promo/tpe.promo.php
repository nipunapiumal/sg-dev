...
#>"><#
	...
	if (settings.images.length > 0) {
		...
		if (settings.type == 'modern') {
			if (settings.link2.url != '' && settings.link2_text != '') {
				var link_atts2 = trx_addons_get_link_attributes( settings.link2 );
				#><a class="sc_promo_link2" href="{{ settings.link2.url }}"{{{ link_atts2 }}}><#
	...
	if ( 'tiny' == settings.size ) {
		if (settings.link.url != '') {
			...
			#><a href="{{ settings.link.url }}"{{{ link_atts }}} class="sc_promo_link"></a><#
		}
	}
#></div><#
