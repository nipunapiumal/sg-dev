...
if ( ! function_exists( 'trx_addons_sc_hotspot' ) ) {
	function trx_addons_sc_hotspot( $atts, $content = '' ) {	
		$atts = trx_addons_sc_prepare_atts( 'trx_sc_hotspot', $atts, trx_addons_sc_common_atts( 'trx_sc_hotspot', 'id,title', array(
			...
			"image_link" => "",
			"spots" => "",
			"popup_title_html_tag" => "h5",
			"popup_subtitle_html_tag" => "h6",
			"popup_price_html_tag" => "div",
			"popup_description_html_tag" => "div",
			"popup_close_icon" => "",
		) ) );
		...
		if ( ! empty( $atts['image'] ) ) {
			...
			trx_addons_get_template_part( array(
											TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $atts['type'] ) ) . '.php',
