...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['actions'] as $item) {
		...
		if (empty($item['color_2'])) $item['color_2'] = '';
		if (empty($item['color_3'])) $item['color_3'] = '';
		...
		?><div class="<?php
			echo apply_filters(
					...
					'sc_action_item sc_action_item_' . esc_attr( $args['type'] )
		...
		?>>
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				...
				?>"<?php
					echo trx_addons_get_link_attributes( $item );
				?>><?php 
