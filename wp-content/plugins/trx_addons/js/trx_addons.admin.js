	...
	$document.ready( function() {
		...
		if ( $body.hasClass( 'trx_addons_legacy_theme_version' ) ) {
			jQuery(         '#trx_addons_theme_panel_section_skins a.trx_addons_image_block_link_download_skin'
					...
					+ ',' + '#upgrade-skins-2[class*="_upgrade_skins_button"]:not([disabled])'
			).on( 'click', function(e) {
					e.preventDefault();
					e.stopImmediatePropagation();
					trx_addons_msgbox_attention( TRX_ADDONS_STORAGE['msg_legacy_theme'], TRX_ADDONS_STORAGE['msg_legacy_theme_caption'] );
					return false;
				}
			);
		}
