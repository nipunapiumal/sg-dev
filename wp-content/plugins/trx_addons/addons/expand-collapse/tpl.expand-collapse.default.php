...
$styles_allowed = apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'expand_collapse' );

?><a href="#" role="button" class="trx_expcol_button<?php
	if ( ! empty( $args['trx_expcol_collapsed_bg_color'] ) || ! empty( $args['trx_expcol_expanded_bg_color'] ) ) {
		?> trx_expcol_button_with_bg<?php
	}
	if ( ! empty( $args['trx_expcol_collapsed_shadow_box_shadow_type'] ) || ! empty( $args['trx_expcol_expanded_shadow_box_shadow_type'] ) ) {
		?> trx_expcol_button_with_shadow<?php
	}
	if ( $styles_allowed ) {
		?> sc_button_default trx_addons_customizable<?php
	}
	?>"
