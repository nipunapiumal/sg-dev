<?php
/**
 * The style "qw-card" of the Services
 *
...
$add_to_cart_link = ! empty( $meta['price'] ) 
						? apply_filters( 'trx_addons_filter_cpt_add_to_cart_link', '', 'sc_services_item_price_link', apply_filters( 'trx_addons_filter_cpt_add_to_cart_label', $meta['price'], 'services-qw-card' ) )
						: '';
...
>
	...
	<?php do_action( 'trx_addons_action_services_item_header_end', $args );
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters( 'trx_addons_filter_args_featured', array(
																...
																'thumb_size' => trx_addons_get_thumb_size($args['columns'] > 1 ? $tpl.'-thumb-square' : 'full'),
																...
																'post_info' => apply_filters('trx_addons_filter_post_info',
																				...
																				. ( ! empty( $link )
																					? '<a class="post_link sc_services_item_link" href="' . esc_url( $link ) . '"' . ( ! empty( $meta['link'] ) && trx_addons_is_external_url( $meta['link'] ) ? trx_addons_external_links_target() : '') . '></a>'
																					: ''
																					),
	...
	if ( (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) || (!empty($link) && !empty($args['more_text']) ) ) { ?>
		<div class="sc_services_item_info"><?php
