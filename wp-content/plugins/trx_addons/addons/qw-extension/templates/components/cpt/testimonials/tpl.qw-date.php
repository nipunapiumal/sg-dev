<?php
/**
 * The style "QW Date" of the Testimonials
 *
...
if ($query->post_count > 0) {
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		if ($args['slider']) {
			...
			$use_masonry = isset( $args['use_masonry'] ) && trx_addons_is_on( $args['use_masonry'] );
			if ( $use_masonry ) {
				trx_addons_enqueue_masonry();
				?><div class="sc_testimonials_masonry_wrap sc_item_masonry sc_item_posts_container columns_padding_bottom <?php
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			trx_addons_get_template_part(array(
											TRX_ADDONS_PLUGIN_CPT . 'testimonials/tpl.' . trx_addons_esc( trx_addons_sanitize_file_name( $args['type'] ) ) . '-item.php',
