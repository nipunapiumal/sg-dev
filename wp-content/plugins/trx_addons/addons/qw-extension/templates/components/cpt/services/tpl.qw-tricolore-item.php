<?php
/**
 * The style "qw-tricolore" of the Services item
 *
...
if ( false && has_post_thumbnail() ) { // background image output is stopped (below)
...
?>>
	...
	<div class="sc_services_item_content <?php echo 1 == $args['hide_bg_image'] ? ' without_image' : ''; ?>"<?php if (false && $args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		...
		<div class="sc_services_item_content_inner">
			...
			<div class="sc_services_item_content_inner_top">
				<?php
				...
				?><div class="sc_services_item_decor"></div>
			</div>
			<div class="sc_services_item_content_inner_bottom"><?php 
			if ($args['featured'] != 'none') {
				if ( has_post_thumbnail() && $args['featured']=='image') {
					?><div class="sc_services_item_thumb" style="background-image: url(<?php
						$trx_addons_attachment_src = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size( $tpl.'-thumb-tiny' ) );
				...
				// Icon
					...
					if (trx_addons_is_url($meta['icon'])) {
						...
						$svg_present = true;
					echo !empty($link) 
						? '<a href="'.esc_url($link).'"'.((!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) ? trx_addons_external_links_target() : '') 
						: '<span';
			...
			?>
			<h2 class="sc_services_item_title"><?php
				if (!empty($link)) {
					?><a href="<?php echo esc_url($link); ?>"<?php if (!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) echo trx_addons_external_links_target(); ?>><?php
				}
				the_title();
				if (!empty($link)) {
					?></a><?php
			...
			if ( ( !isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0 ) || ( !empty($meta['price']) ) || ( !empty($link) && !empty($args['more_text']) ) ) {
			?><div class="sc_services_item_info_wrap"><?php
				...
				if (!empty($link) && !empty($args['more_text'])) {
					?><div class="sc_services_item_button sc_item_button">
						<a href="<?php echo esc_url($link); ?>"<?php if (!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) echo trx_addons_external_links_target(); ?> class="sc_services_item_more_link">
		...
		if (!empty($link)) {
			?><a class="sc_services_item_link" href="<?php echo esc_url($link); ?>"<?php if (!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) echo trx_addons_external_links_target(); ?>></a><?php
		}
