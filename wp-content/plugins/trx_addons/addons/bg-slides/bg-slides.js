	...
	$document.on('action.init_hidden_elements', function( e, cont ) {
		if ( cont.hasClass( 'elementor-section' ) || cont.hasClass( 'e-con' ) ) {
			...
			jQuery( ( typeof window.elementorFrontend !== 'undefined' && elementorFrontend.isEditMode()
						? '.elementor-section.elementor-element-edit-mode,.e-con.elementor-element-edit-mode'
			...
			if ( output ) {
				...
				var trx_addons_bg_slides_set_wrap_dimensions = function() {
					...
					if ( $wrap.hasClass( 'trx_addons_bg_slides_fixed' ) ) {
						wrap_offset.top = trx_addons_fixed_rows_height();
					}
					$wrap.data( {
								'trx-addons-bg-slides-offset': wrap_offset,
