...
( function() {
	...
	function spa_replace_page( href ) {
		...
		if ( $html_wrapper.length == 1 ) {
			...
			var page_title_selector = trx_addons_apply_filters( 'trx_addons_filter_page_title_selector', '.sc_layouts_title_caption,head title' );
			var title = $html.find( page_title_selector ).eq(0).text() || '';
			jQuery( page_title_selector ).html( title );
