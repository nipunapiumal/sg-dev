...
class Logger extends Singleton {
	...
	public function log( $response, $type = 'chat', $args = array(), $section = '' ) {
		...
		if ( ! isset( $this->log[ $section ] ) ) {
			$this->log[ $section ] = array();
		}
		...
		// Image generation: ModelsLab (ex Stable Diffusion) API & Stability AI
		...
		// Image to Layout
		} else if ( $type == 'image-to-layout' ) {
			$subtype = 'default';
			if ( empty( $this->log[ $section ][ 'image-to-layout' ] ) ) {
				$this->log[ $section ][ 'image-to-layout' ] = array();
			}
			if ( empty( $this->log[ $section ][ 'image-to-layout' ][ $subtype ] ) ) {
				$this->log[ $section ][ 'image-to-layout' ][ $subtype ] = 0;
			}
			$this->log[ $section ][ 'image-to-layout' ][ $subtype ]++;

		// Speech To Text
		} else if ( $type == 'speech-to-text' ) {
			$subtype = 'default';
			if ( empty( $this->log[ $section ][ 'speech-to-text' ] ) ) {
				$this->log[ $section ][ 'speech-to-text' ] = array();
			}
			if ( empty( $this->log[ $section ][ 'speech-to-text' ][ $subtype ] ) ) {
				$this->log[ $section ][ 'speech-to-text' ][ $subtype ] = 0;
			}
			$this->log[ $section ][ 'speech-to-text' ][ $subtype ]++;

		// Video generation
		} else if ( in_array( $section, array( 'lumalabs-ai' ) ) ) {
			$number = ! empty( $args['n'] ) ? (int)$args['n'] : 1;
			$subtype = 'default';
			if ( empty( $this->log[ $section ][ $type ] ) ) {
				$this->log[ $section ][ $type ] = array();
										...
										.  ( empty( $section )
											...
											: ( in_array( $section, array( 'open-ai', 'x-ai' ) )
												...
												: ( in_array( $section, array( 'open-ai-assistants', 'flowise-ai', 'google-ai' ) )
													...
													: ( $section == 'lumalabs-ai'
														? __( 'Generated videos:', 'trx_addons' )
