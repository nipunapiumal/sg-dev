...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		...
		private function get_site_logo( $args ) {
			...
			return array(
				...
				'message' => sprintf(
								...
								$args['type'],
								$args['location'],
								empty( $logo_url ) ? __( 'not selected', 'trx_addons' ) : '<a href="' . esc_url( $logo_url ) . '"' . trx_addons_external_links_target( true ) . '>' . trx_addons_get_file_name( $logo_url, false ) . '</a>'
							),
				'value' => $logo_url
			);
		}
		...
		private function set_site_logo( $args ) {
			...
			return array(
				...
				'message' => sprintf(
								...
								$args['type'],
								$args['location'],
								'<a href="' . esc_url( $logo_url ) . '"' . trx_addons_external_links_target( true ) . '>' . trx_addons_get_file_name( $logo_url, false ) . '</a>'
							),
				//'value' => $logo_url
			);
		}
		...
		private function set_scheme_color( $args ) {
			...
			// Check if a new value is a correct hex color value
			if ( substr( $args['value'], 0, 1 ) == '#' && ! preg_match( '/^#[0-9a-f]{3}([0-9a-f]{3})?$/i', $args['value'] ) ) {
