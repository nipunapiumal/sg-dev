...
class InfoBoxWidget extends BaseWidget {
	...
	protected function register_style_icon_controls() {
		...
		$this->add_control(
			'hover_animation_rotate_angle',
			array(
				'label'     => __( 'Angle', 'trx_addons' ),
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .elementor-animation-rotate:active,
					 {{WRAPPER}} .elementor-animation-rotate:focus,
					 {{WRAPPER}} .elementor-animation-rotate:hover,
					 {{WRAPPER}}.with-hover-on-widget:hover .elementor-animation-rotate' => 'transform: rotate({{VALUE}}deg);',
				),
				'condition' => [
					'hover_animation_icon' => 'rotate',
				],
			)
		);
	...
	protected function content_template() {
		...
		<div {{{ view.getRenderAttributeString( 'info-box-container' ) }}}><#
			...
			if ( settings.link.url != '' && settings.link_type == 'box' ) {
				var link_atts = trx_addons_get_link_attributes( settings.link );
				#><a class="trx-addons-info-box-container-link" href="{{ settings.link.url }}"{{{ link_atts }}}></a><#
			}
				...
				<# if ( settings.icon_type != 'none' ) { #>
					<div class="trx-addons-info-box-icon-wrap trx-addons-info-box-icon-type-{{ settings.icon_type }}">
						<# if ( settings.link_type == 'icon' ) {
							var link_atts = trx_addons_get_link_attributes( settings.link );
							#>
							<a href="{{ _.escape( settings.link.url ) }}"{{{ link_atts }}}>
						<# } #>
							...
							<span class="trx-addons-info-box-icon-inner elementor-animation-{{ settings.hover_animation_icon }}">
								...
								if ( settings.icon_type == 'icon' ) {
									if ( settings.icon || settings.selected_icon ) {
										if ( iconHTML && iconHTML.value && ( ! settings.icon || migrated ) ) {
											#>
											{{{ iconHTML.value }}}
											<#
										} else {
				...
				<div class="trx-addons-info-box-content">
					...
					<# if ( settings.link_type == 'button' || ( settings.link_type == 'box' && settings.button_visible == 'yes' ) ) { #>
						<# if ( settings.button_text != '' || settings.button_icon != '' ) {
							var link_atts = settings.link.url ? trx_addons_get_link_attributes( settings.link ) : '';
							#>
							<div class="trx-addons-info-box-footer">
								<{{trx_addons_button_html_tag}} href="{{ settings.link.url }}"{{{ link_atts }}} class="trx-addons-info-box-button trx-addons-info-box-button-icon-{{ settings.button_icon_position }} elementor-button elementor-animation-{{ settings.button_animation }}">
