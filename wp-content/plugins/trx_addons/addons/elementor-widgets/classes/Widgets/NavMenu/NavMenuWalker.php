...
class NavMenuWalker extends \Walker_Nav_Menu {
	...
	public function get_default_submenu_icon( $depth ) {
		...
		// Don't show default submenu icons if user has disabled them (select 'None')
		return '';
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( ! in_array( 'trx-addons-nav-menu-item', $classes ) ) {
			$classes[] = 'trx-addons-nav-menu-item';
		}

		if ( 0 < $depth && ! in_array( 'trx-addons-submenu-item', $classes ) ) {
			$classes[] = 'trx-addons-submenu-item';
		}

		if ( (int)$item_meta->layout_submenu > 0 ) {
			if ( ! in_array( 'trx-addons-mega-nav-item', $classes, true ) ) {
				$classes[] = 'trx-addons-mega-nav-item';
			}
			if ( ! in_array( 'menu-item-has-children', $classes, true ) ) {
				$classes[] = 'menu-item-has-children';
			}
			if ( ! in_array( 'menu-item-has-children-layout', $classes, true ) ) {
				$classes[] = 'menu-item-has-children-layout';
			}

			if ( ! empty( $item_meta->submenu_pos ) && $item_meta->submenu_pos == 'static' && ! in_array( 'trx-addons-mega-item-static', $classes ) ) {
				$classes[] = 'trx-addons-mega-item-static';
			}
		}

		// we can later add other classes here based on the user settings.
		if ( in_array( 'current-menu-item', $classes ) && ! in_array( 'trx-addons-active-item', $classes ) ) {
			$classes[] = 'trx-addons-active-item';
		}

		// Add badge marker.
		if ( ! empty( $item_meta->badge_text ) ) {
			if ( ! in_array( 'has-trx-addons-badge', $classes ) ) {
			...
			if ( 0 < $depth && '' !== $badge_effect && ! in_array( 'trx-addons-badge-' . $badge_effect, $classes ) ) {
