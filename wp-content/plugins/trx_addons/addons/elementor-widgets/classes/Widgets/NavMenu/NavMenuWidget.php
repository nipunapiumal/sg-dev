...
class NavMenuWidget extends BaseWidget {
	...
	private function register_style_submenu_container_controls() {
		...
		$this->add_responsive_control(
			'sub_maxheight',
			array(
				'label'       => __( 'Maximum Height', 'trx_addons' ),
				'type'        => Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%', 'em', 'rem', 'vw', 'vh', 'custom' ],
				'range'       => array(
					'px' => array(
				...
				'selectors'   => array(
					'{{WRAPPER}} .trx-addons-mobile-menu-container .trx-addons-submenu,
					{{WRAPPER}}.trx-addons-nav-ver .trx-addons-nav-menu-item.menu-item-has-children .trx-addons-submenu,
					{{WRAPPER}}.trx-addons-nav-hor .trx-addons-nav-menu-item.menu-item-has-children .trx-addons-submenu' => 'max-height: {{SIZE}}{{UNIT}};overflow: hidden auto;',
				),
				'condition'   => array(
					'nav_menu_layout' => 'hor'
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'   => array(
					'{{WRAPPER}}.trx-addons-nav-hor .trx-addons-nav-menu-container .trx-addons-mega-content-container .sc_layouts_submenu_wrap' => 'max-height: {{SIZE}}{{UNIT}};overflow: hidden auto;',
				),
				'condition'   => array(
					'nav_menu_layout' => 'hor'
				),
			...
			<div class="trx-addons-ver-inner-container">
				...
				if ( 'wordpress_menu' === $menu_type ) {
					$args = array(
						...
						'menu_id'     => 'trx-addons-nav-menu-' . $menu_id . '-' . mt_rand( 1000, 9999 ),
						'echo'        => false,
	...
	private function mobile_menu_filter( $menu_html, $menu_id ) {
		...
		$slug  = 'trx-addons-nav-menu-' . $menu_id;
		$slug2 = 'trx-addons-nav-menu-' . $menu_id . '-' . mt_rand( 1000, 9999 );
		if ( preg_match( '/<ul id="(' . $slug . '[\-0-9]*)"/', $menu_html, $matches ) ) {
			$slug = $matches[1];
		}
		$search  = array( 'id="' . $slug  . '"', 'class="trx-addons-nav-menu trx-addons-main-nav-menu"' );
		$replace = array( 'id="' . $slug2 . '"', 'class="trx-addons-mobile-menu trx-addons-main-mobile-menu trx-addons-main-nav-menu"' );
