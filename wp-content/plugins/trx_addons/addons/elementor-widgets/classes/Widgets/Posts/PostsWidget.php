...
class PostsWidget extends BaseWidget {
	...
	protected function register_skins() {
		...
		$this->add_skin( new Skins\SkinStripe( $this ) );
	}
	...
	public function register_query_section_controls( $condition = array(), $widget_type = 'posts', $old_code = '', $advanced_controls = 'no' ) {
		...
		foreach ( $post_types as $post_type_slug => $post_type_label ) {
			...
			if ( ! empty( $taxonomies[ $post_type_slug ] ) ) {
				...
				foreach ( $taxonomies[ $post_type_slug ] as $index => $tax ) {
					...
					$terms = ! $is_edit_mode ? array(1) : TrxAddonsUtils::get_tax_terms( $index );
		...
		$this->add_control(
			...
			array(
				...
				'default'     => '',
				'multiple'    => true,
				'options'     => ! $is_edit_mode ? array() : TrxAddonsUtils::get_taxonomies_list(),
				'condition'   => array(
					'query_type'         => 'custom',
					'post_type'          => 'related',
