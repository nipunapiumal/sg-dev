...
abstract class BaseSkin extends Elementor_Skin_Base {
	...
	protected function register_terms_controls() {
		...
		foreach ( $post_types as $post_type_slug => $post_type_label ) {
			...
			if ( ! isset( $taxonomies[ $post_type_slug ] ) ) {
				$taxonomies[ $post_type_slug ] = TrxAddonsUtils::get_post_taxonomies( $post_type_slug );
			}

			if ( ! empty( $taxonomies[ $post_type_slug ] ) ) {

				$related_tax = array();

				// Get all taxonomy values under the taxonomy.
				foreach ( $taxonomies[ $post_type_slug ] as $index => $tax ) {
	...
	protected function register_image_controls() {
		...
		$this->add_control(
			'thumbnail_over_content_on_mobile',
			array(
				'label'     => __( 'Image Over Content on mobile', 'trx_addons' ),
				'label_block' => false,
				'type'      => Controls_Manager::SWITCHER,
				'prefix_class' => 'trx-addons-posts-image-over-content-on-mobile-',
				'condition' => array(
	...
	protected function register_excerpt_controls() {
		...
		$this->add_control(
			'hide_excerpt_on_mobile',
			array(
				'label'     => __( 'Hide Content on mobile', 'trx_addons' ),
				'label_block' => false,
				'type'      => Controls_Manager::SWITCHER,
				'prefix_class' => 'trx-addons-posts-hide-content-on-mobile-',
				'condition' => array(
	...
	protected function register_style_layout_controls() {
		...
		$this->add_responsive_control(
			'first_column_width',
			array(
				'label'     => __( 'First Content Column Width (%)', 'trx_addons' ),
				'type'      => Controls_Manager::SLIDER,
				'size_units' => [ '%' ],
				'selectors' => array(
					'{{WRAPPER}} .trx-addons-posts-skin-stripe .trx-addons-posts-item-content-column-one' => 'width: {{SIZE}}%;',
					'{{WRAPPER}} .trx-addons-posts-skin-stripe .trx-addons-posts-item-content-column-two' => 'width: calc( 100% - {{SIZE}}% );',
				),
				'condition' => array(
					'_skin' => array( 'stripe' ),
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .trx-addons-posts-skin-stripe .trx-addons-posts-item-content' => 'gap: {{SIZE}}{{UNIT}};',
				),
				'condition' => array(
					'_skin' => array( 'event', 'stripe' ),
				),
			)
		);
	...
	protected function register_style_button_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .trx-addons-posts-button  .trx-addons-button-icon' => 'color: {{VALUE}}',
					'{{WRAPPER}} .trx-addons-posts-button  .trx-addons-button-icon svg' => 'fill: {{VALUE}}',
				),
				'condition' => array(
					$this->get_control_id( 'show_button' ) => 'yes',
					$this->get_control_id( 'select_button_icon[value]!' ) => '',
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .trx-addons-posts-button:hover  .trx-addons-button-icon' => 'color: {{VALUE}}',
					'{{WRAPPER}} .trx-addons-posts-button:hover  .trx-addons-button-icon svg' => 'fill: {{VALUE}}',
				),
				'condition' => array(
					$this->get_control_id( 'show_button' ) => 'yes',
					$this->get_control_id( 'select_button_icon[value]!' ) => '',
	...
	public function get_ordered_items( $items ) {
		...
		foreach ( $items as $item ) {
			...
			if ( empty( $order ) || ! is_numeric( $order ) || $order < 1 ) {
				$order = 1;
			}
	...
	public function remove_class_sticky_from_posts( $classes ) {
		return array_diff( $classes, array( 'sticky' ) );
	}
	...
	public function render() {
		...
		<div <?php echo wp_kses_post( $this->parent->get_render_attribute_string( 'posts-container' ) ); ?>>
			...
			<div <?php echo wp_kses_post( $this->parent->get_render_attribute_string( 'posts-wrap' ) ); ?>>
				...
				add_filter( 'post_class', [ $this, 'remove_class_sticky_from_posts' ], 9999 );
				...
				remove_filter( 'post_class', [ $this, 'remove_class_sticky_from_posts' ], 9999 );
			...
			<div class="<?php echo esc_attr( $this->get_item_classes() ); ?>">
				...
				<div class="trx-addons-posts-item-content-wrap">
					<div class="trx-addons-posts-item-content"><?php
						$skin = $this->get_id();
						$content_parts = $this->get_ordered_items( Posts::get_post_parts( 'stripe' == $skin ) );

						if ( $skin == 'stripe' ) {
							?>
							<div class="trx-addons-posts-item-content-column-one">
								<div class="trx-addons-posts-item-content-column-start">
							<?php
						}
						...
						foreach ( $content_parts as $part => $index ) {
							...
							if ( $skin == 'stripe' && $part_counter > 0 && $part !== 'terms' ) {
								$part_depth--;
								?>
								</div>
								<?php
								if ( $part_counter < count( $content_parts ) ) {
									if ( $part_counter == 1 || $part_counter == 3 ) {
										$part_depth++;
										?>
										<div class="trx-addons-posts-item-content-column-end">
										<?php
									} else if ( $part_counter == 2 ) {
										$part_depth++;
										?>
										</div>
										<div class="trx-addons-posts-item-content-column-two">
											<div class="trx-addons-posts-item-content-column-start">
										<?php
						...
						if ( $skin == 'stripe' && $part_depth > 0 ) {
							for ( $i = 0; $i < $part_depth; $i++ ) {
								?>
								</div>
								<?php
							}
	...
	protected function render_button_icon() {
		...
		if ( $is_new || $migrated ) {
			if ( ! empty( $select_button_icon['value'] ) ) {
				?>
				<span class="trx-addons-button-icon trx-addons-icon elementor-button-icon elementor-align-icon-<?php echo esc_attr( $button_icon_align ); ?>">
					<?php Icons_Manager::render_icon( $select_button_icon, array( 'aria-hidden' => 'true' ) ); ?>
				</span>
				<?php
			}
		} else if ( ! empty( $button_icon ) ) {
			?>
