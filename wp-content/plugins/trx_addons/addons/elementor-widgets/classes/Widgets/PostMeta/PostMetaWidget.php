	...
	protected function register_content_post_meta_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'description' => sprintf( '<a href="https://wordpress.org/documentation/article/customize-date-and-time-format/"' . trx_addons_external_links_target( true ) . '>%s</a>', esc_html__( 'Documentation on date and time formatting', 'trx_addons' ) ),
				'condition' => [
					'date_format' => 'custom',
					'type'        => ['date', 'modified']
				],
	...
	protected function register_style_post_meta_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .trx-addons-post-meta' => 'gap: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render_meta_item( $meta ) {
		...
		if ( ! empty(  $meta['type'] ) && $meta['type'] !== 'none' ) {
			...
			if ( $meta['type'] === 'author' ) {
				...
				if ( ! empty( $meta['type_custom'] ) ) {
					$output .= wp_kses_post( get_post_meta( get_the_ID(), $meta['type_custom'], true ) );
				}
