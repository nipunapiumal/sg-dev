...
class PricingTableWidget extends BaseWidget {
	...
	protected function register_content_pricing_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type'                  => Controls_Manager::SELECT,
				'options'               => apply_filters( 'trx_addons_filter_pricing_table_currencies', [
					''             => __( 'None', 'trx_addons' ),
	...
	protected function register_style_features_controls() {
		...
		// $this->add_responsive_control(
		// 	'table_features_elements_spacing',
		// 	[
		...
		// 		],
		// 		'selectors'             => [
		// 			'{{WRAPPER}} .trx-addons-pricing-table-features .trx-addons-pricing-table-feature-text > :not(:last-child)' => 'margin-bottom: {{SIZE}}{{UNIT}};',
		// 		],
		// 	]
		// );
	...
	protected function register_style_footer_controls() {
		...
		$this->add_responsive_control(
			'additional_info_elements_spacing',
			[
				'label'                 => __( 'Elements Spacing', 'trx_addons' ),
				'type'                  => Controls_Manager::SLIDER,
				'size_units'            => [ 'px', '%', 'em', 'rem', 'vw', 'vh', 'custom' ],
				...
				'selectors'             => [
					'{{WRAPPER}} .trx-addons-pricing-table-additional-info > :not(:last-child)' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		<div class="trx-addons-pricing-table-container">
			<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'pricing-table' ) ); ?>>
				...
				<?php if ( count( $settings['table_features'] ) > 0
							&& ( ! empty( $settings['table_features'][0]['feature_text'] )
								|| ! empty( $settings['table_features'][0]['feature_icon'] )
								|| ! empty( $settings['table_features'][0]['select_feature_icon']['value'] )
								)
				) { ?>
	...
	protected function content_template() {
					...
					<a href="{{{ _.escape( settings.link.url ) }}}" {{{ view.getRenderAttributeString( 'table_button_text' ) }}}>
						<# if ( settings.button_icon_position == 'before' ) { #>
							<# if ( settings.button_icon || settings.select_button_icon.value ) { #>
								<span class="trx-addons-button-icon trx-addons-icon">
									<# if ( buttonIconHTML && buttonIconHTML.value && ( ! settings.button_icon || buttonMigrated ) ) { #>
		...
		<div class="trx-addons-pricing-table-container">
			<div class="trx-addons-pricing-table trx-addons-pricing-table-currency-{{ settings.currency_format }} trx-addons-pricing-table-tooltip-{{ settings.show_tooltip }}">
						...
						<div class="trx-addons-pricing-table-icon-wrap">
							<# if ( settings.icon_type == 'icon' ) {
								if ( settings.table_icon || settings.select_table_icon ) {
									...
									#>
									<span class="trx-addons-pricing-table-icon trx-addons-icon">
										<# if ( iconHTML && iconHTML.value && ( ! settings.table_icon || migrated ) ) { #>
											{{{ iconHTML.value }}}
										<# } else { #>
				...
				<# if ( settings.table_features.length
					&& settings.table_features[0]
					&& (   settings.table_features[0].feature_text
						|| settings.table_features[0].feature_icon
						|| settings.table_features[0].select_feature_icon.value
						)
				) { #>
								...
								<div {{{ view.getRenderAttributeString( featureContentKey ) }}}>
									<# if ( item.feature_icon || item.select_feature_icon.value ) { #>
										...
										<#
											...
											if ( iconsHTML[ index ] && iconsHTML[ index ].value && ( ! item.feature_icon || iconsMigrated[ index ] ) ) { #>
