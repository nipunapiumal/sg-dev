				...
				if ( ! empty( $params['required'] ) ) {
					...
					if ( ( $fdir = trx_addons_get_file_dir( TRX_ADDONS_PLUGIN_ADDONS . trx_addons_esc( "{$w}/{$w}.php" ) ) ) != '') { 
						if ( ! empty( $activated[ $w ] ) || ! isset( $activated[ $w ] ) ) {
							...
							$need_update = empty( $activated[ $w ] ) ? true : $need_update;
							$activated[ $w ] = true;
						}
...
if ( ! function_exists( 'trx_addons_get_available_addons' ) ) {
	...
	function trx_addons_get_available_addons( $addons = array() ) {
		...
		if ( ! is_array( $addons ) || count( $addons ) == 0 || ! empty( $_GET['force-check'] ) ) {
			...
			set_transient( 'trx_addons_list_addons', $addons, 2 * 24 * 60 * 60 );       // Store to the cache for 2 days
		}
								 	...
								 	<div class="trx_addons_image_block_footer">
										...
										if ( ! empty( $data['title'] ) ) {
											...
											<h5 class="trx_addons_image_block_title">
												...
												?>
												<a href="<?php echo esc_url( trx_addons_theme_panel_get_addon_doc_url( $addon ) ); ?>"<?php echo trx_addons_external_links_target( true ); ?> class="trx_addons_image_block_doc_link trx_addons_icon-help-circled" title="<?php esc_html_e( 'Addon Documentation (opens in a new window)', 'trx_addons' ); ?>"></a>
											</h5>
											<?php
										}
										// Addon description
...
if ( ! function_exists( 'trx_addons_theme_panel_get_addon_doc_url' ) ) {
	function trx_addons_theme_panel_get_addon_doc_url( $addon ) {
		$substitutes = array(
			'bg_colors' => 'background_colors',
			'bg_canvas' => 'dynamic_background',
			'cpt_to_cart' => 'cpt_cart',
		);
		$addon = str_replace( '-', '_', $addon );
		if ( isset( $substitutes[ $addon ] ) ) {
			$addon = $substitutes[ $addon ];
		}
		return trx_addons_get_theme_doc_url( '#theme_addons_' . $addon );
	}
}
	...
	function trx_addons_ajax_download_addons() {
		...
		if ( ! empty( $TRX_ADDONS_STORAGE['addons_required'] ) ) {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				...
				$addon  = trx_addons_sanitize_slug( trx_addons_get_value_gp( 'addon' ) );
