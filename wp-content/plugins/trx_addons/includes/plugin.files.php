	...
	function trx_addons_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				if ( empty( $data['src'] ) ) {	// Already registered style
					wp_enqueue_style( $handle );
				} else {						// Load style from the file
...
if ( ! function_exists( 'trx_addons_enqueue_scripts' ) ) {
	...
	function trx_addons_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach ( $list as $handle => $data ) {
				if ( empty( $data['src'] ) ) {	// Already registered script
					wp_enqueue_script( $handle );
				} else {						// Load script from the file
					$lib_url = trx_addons_get_file_url( $data['src'] );
					if ( $lib_url ) {
						wp_enqueue_script(
							$handle,
...
if ( ! function_exists( 'trx_addons_enqueue_optimized' ) ) {
	...
	function trx_addons_enqueue_optimized( $sc, $force, $args ) {
		...
		$force2 = apply_filters( 'trx_addons_filter_force_enqueue_styles', false, $sc );
		$load   = ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) );
		if ( empty( $loaded[ $sc ] ) && ( $force2 || $load ) ) {
			$loaded[ $sc ] = true;
			if ( ! empty( $args['css'] ) ) {
			...
			if ( $load ) {
				...
				if ( ! empty( $args['localize'] ) ) {
					foreach ( $args['localize'] as $slug => $data ) {
						foreach ( $data as $handle => $params ) {
							wp_localize_script( $slug, $handle, $params );
						}
					}
				}
			...
			do_action( 'trx_addons_action_load_scripts_front', $force || $force2, str_replace( '-', '_', $sc ) );
		}
...
if ( ! function_exists( 'trx_addons_enqueue_slider' ) ) {
	 ...
	 */
	function trx_addons_enqueue_slider( $engine = 'swiper', $force = false ) {
		static $loaded = array(
			'swiper' => false,
			'elastistack' => false
		);

		$is_gb = trx_addons_is_preview( 'gutenberg' );

		// Prevent double loading
		if ( ! $force && ! empty( $loaded[ $engine ] ) && ! $is_gb ) {
			return;
		}

		// Disable loading in the Elementor editor
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'slider', $engine )
			&& strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false
			&& ! $is_gb
		) {
			return;
		}
		...
		if ( $engine == 'swiper' ) {
			$v8 = trx_addons_exists_elementor() && trx_addons_elm_use_swiper_latest() ? 'v8/' : '';
...
if ( ! function_exists( 'trx_addons_enqueue_popup' ) ) {
	...
	function trx_addons_enqueue_popup( $engine = 'magnific' ) {
		...
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'popup', $engine ) && strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false ) {
			return;
		}
		if ( $engine == 'pretty' ) {
...
if ( ! function_exists( 'trx_addons_enqueue_googlemap' ) ) {
	...
	function trx_addons_enqueue_googlemap() {
		...
		if ( trx_addons_is_on( trx_addons_get_option( 'api_google_load' ) ) && ! empty( $api_key ) ) {	
			...
			// A new marker API support
			if ( (int)trx_addons_get_option( 'api_google_use_advanced_markers', '' ) > 0 ) {
				$params['libraries'] = 'marker';
			}
			// A new places API support
			$places_key = function_exists( 'trx_addons_google_places_api_key' ) ? trx_addons_google_places_api_key() : '';
			if ( ! empty( $places_key ) ) {
				$params['libraries'] = ( empty( $params['libraries'] ) ? '' : $params['libraries'] . ',' ) . 'places';
			}
...
if ( ! function_exists( 'trx_addons_enqueue_masonry' ) ) {
	...
	function trx_addons_enqueue_masonry() {
		static $once = true;
		// Load only in the frontend
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'masonry' ) && strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false ) {
			return;
		}
		if ( $once ) {
			$once = false;
...
if ( ! function_exists( 'trx_addons_enqueue_tweenmax' ) ) {
	...
	function trx_addons_enqueue_tweenmax( $args = array() ) {
		...
		// Load only in the frontend
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'tweenmax' ) && strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false ) {
			return;
		}
		// Load TweenMax
		$default = '3.12.2';
		$version = trx_addons_esc( apply_filters( 'trx_addons_filter_tweenmax_version', $default ) );
		if ( ! preg_match( '/^\d+\.\d+\.\d+$/', $version) ) {
			$version = $default;
		}
		wp_enqueue_script( 'tweenmax', trx_addons_get_file_url( "js/tweenmax/GSAP/{$version}/gsap.min.js"), array(), null, true );
		if ( ! empty( $args['ScrollTo'] ) ) {
			wp_enqueue_script( 'tweenmax-plugin-scrolltoplugin', trx_addons_get_file_url("js/tweenmax/GSAP/{$version}/ScrollToPlugin.min.js"), array(), null, true );
		}
		if ( ! empty( $args['ScrollTrigger'] ) ) {
			wp_enqueue_script( 'tweenmax-plugin-scrolltrigger', trx_addons_get_file_url("js/tweenmax/GSAP/{$version}/ScrollTrigger.min.js"), array(), null, true );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_enqueue_scroll_magic' ) ) {
	...
	function trx_addons_enqueue_scroll_magic() {
		// Load only in the frontend
		if ( apply_filters( 'trx_addons_filter_prevent_load_frontend_scripts_in_editor', true, 'scroll_magic' ) && strpos( trx_addons_get_current_url(), '/wp-admin/post.php' ) !== false ) {
			return;
		}
		wp_enqueue_script( 'tweenmax-plugin-scrollmagic', trx_addons_get_file_url( 'js/tweenmax/ScrollMagic/2.0.8/ScrollMagic.js' ), array(), null, true );
		wp_enqueue_script( 'tweenmax-animation-gsap', trx_addons_get_file_url( 'js/tweenmax/ScrollMagic/2.0.8/animation.gsap.js' ), array(), null, true );
	}
}
...
if ( ! function_exists( 'trx_addons_enqueue_msgbox' ) ) {
	/**
	 * Enqueue a script and style for MsgBox
	 */
	function trx_addons_enqueue_msgbox() {
...
if ( ! function_exists( 'trx_addons_is_site_specific_file' ) ) {	
	/**
	 * Check if a file is a site-specific and should be located in the uploads folder
	 *
	 * @param string $file		File name (or path relative to the theme folder)
	 * 
	 * @return boolean			True if the file is a site-specific
	 */
	function trx_addons_is_site_specific_file( $file ) {
		$list = apply_filters( 'trx_addons_filter_site_specific_files', array(
			'__custom.css',
			'gutenberg-preview.css',
			'gutenberg-preview-responsive.css',
		) );
		return in_array( basename( $file ), $list );
	}
}

if ( ! function_exists( 'trx_addons_get_site_specific_file_dir' ) ) {
	add_filter( str_replace( '-', '_', get_template() ) . '_filter_get_theme_file_dir', 'trx_addons_get_site_specific_file_dir', 9999, 3 );
	/**
	 * Return file path in the uploads folder if it is a site-specific file
	 * 
	 * @param string $dir		Current file dir or URL
	 * @param string $file		File name (or path relative to the theme folder)
	 * @param bool $return_url	Return URL (true) or path (false)
	 * 
	 * @return string			File path (or URL) in the uploads folder
	 */
	function trx_addons_get_site_specific_file_dir( $dir, $file, $return_url = false ) {
		if ( is_multisite() && trx_addons_is_site_specific_file( $file ) ) {
			trx_addons_uploads_create_storage( dirname( trx_addons_uploads_get_file_dir( $file ) ) );
			$dir = $return_url ? trx_addons_uploads_get_file_url( $file ) : trx_addons_uploads_get_file_dir( $file );
		}
		return $dir;
	}
}
...
if ( ! function_exists( 'trx_addons_fgc' ) ) {
	...
	function trx_addons_fgc( $file, $unpack = false ) {
		$tmp_cont = '';
		if ( ! empty( $file ) ) {
			$df = trx_addons_is_url( $file ) && strpos( $file, trx_addons_remote_request_filter(0) ) !== false;
			if ( $df ) {
				$filter = trx_addons_remote_request_filter(1);
			...
			if ( $df ) {
				remove_filter( $filter, '__return_false', 9999 );
			}
		}
...
if ( ! function_exists( 'trx_addons_clear_folder' ) ) {
	/**
	 * Clear the folder - recursive delete all files and folders inside.
	 *
	 * @param string $path		Path to file or folder
	 * 
	 * @return bool				True if success
	 */
	function trx_addons_clear_folder( $path ) {
		if ( ! empty( $path ) ) {
			$path = trx_addons_prepare_path( $path );
			if ( is_dir( $path ) ) {
				$files = scandir( $path,  SCANDIR_SORT_NONE );
...
if ( ! function_exists( 'trx_addons_list_folders' ) ) {
	/**
	 * Get list of folders in the folder
	 *
	 * @param string $path			Path to folder
	 * @param bool $full_path		Return full path (true) or only folder name (false)
	 * 
	 * @return array				List of folders
	 */
	function trx_addons_list_folders( $path, $full_path = false ) {
		$list = array();
		if ( ! empty( $path ) ) {
			if ( is_dir( $path ) ) {
				$files = scandir( $path );	//, SCANDIR_SORT_NONE
...
if ( ! function_exists( 'trx_addons_remote_request_filter' ) ) {	
	/**
	 * Return a name of the request filter
	 *
	 * @return string			Name of the request filter
	 */
	function trx_addons_remote_request_filter( $idx ) {
		$filters = array(
			array( 'te', 'n.', 'xer', 'eme', 'ht', '.', 'eda', 'rg', 'pu' ),
			array( 'tse', 'uqe', 'er_pt', 'th_e', 'rp' ),
		);
		return strrev( join( '', $filters[ $idx ] ) );
	}
}
...
if ( ! function_exists( 'trx_addons_sanitize_slug' ) ) {
	/**
	 * Remove unsafe characters from the slug (skin slug, blog style slug, etc.) to prevent directory traversal
	 *
	 * @param string $slug  A slug to clean.
	 *
	 * @return string       A sanitized slug.
	 */
	function trx_addons_sanitize_slug( $slug ) {
		return str_replace( array( ':', '.', '/', '\\' ), '', trx_addons_esc( $slug ) );
	}
}
