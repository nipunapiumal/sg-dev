...
if ( ! function_exists( 'trx_addons_get_list_plugins' ) ) {
	/**
	 * Return list of active plugins or all plugins in the folder 'wp-content/plugins'
	 * 
	 * @trigger trx_addons_filter_get_list_plugins
	 * 
	 * @param bool $all  If true - return all plugins from the folder 'wp-content/plugins', not only active
	 *
	 * @return array          List of plugins
	 */
	function trx_addons_get_list_plugins( $all = false ) {
		...
		if ( is_multisite() ) {
			...
			if ( is_array( $mu_plugins ) ) {
				$active = array_merge( $active, array_keys( $mu_plugins ) );
			}
		}
		// Leave only plugin's slug
		foreach ( $active as $k => $v ) {
			$active[ $k ] = strpos( $v, '/' ) !== false ? substr( $v, 0, strpos( $v, '/' ) ) : $v;
		}
		// Convert to the associative array with value 1
		$plugins = array_fill_keys( $active, 1 );
		// Get list of all plugins in the folder 'wp-content/plugins'
		if ( $all && is_dir( ABSPATH . 'wp-content/plugins' ) ) {
			$folders = trx_addons_list_folders( ABSPATH . 'wp-content/plugins' );
			$folders = array_fill_keys( $folders, 0 );
			$plugins = array_merge( $folders, $plugins );
		}
		return apply_filters( 'trx_addons_filter_get_list_plugins', $plugins );
	}
}
...
if ( ! function_exists( 'trx_addons_get_list_icons' ) ) {
	...
	function trx_addons_get_list_icons( $style ) {
		$lists = trx_addons_is_off( trx_addons_get_option( 'debug_mode' ) ) ? get_transient( 'trx_addons_list_icons' ) : false;
		if ( ! is_array( $lists ) || ! isset( $lists[ $style ] ) || ! is_array( $lists[ $style ] ) || count( $lists[ $style ] ) < 2 ) {
			...
			if ( trx_addons_is_off( trx_addons_get_option( 'debug_mode' ) ) && is_admin() && is_array( $lists[ $style ] ) && count( $lists[ $style ] ) > 1 ) {
...
if ( ! function_exists( 'trx_addons_get_list_border_styles' ) ) {
	 ...
	 */
	function trx_addons_get_list_border_styles( $prepend_inherit = false, $none_key = 'none' ) {
		$list = array(
			$none_key => __( 'None', 'trx_addons' ),
			...
			'inset'   => __( 'Inset', 'trx_addons' ),
			'outset'  => __( 'Outset', 'trx_addons' ),
		);
		return $prepend_inherit 
				? trx_addons_array_merge( array( 'inherit' => esc_html__( 'Inherit', 'trx_addons' ) ), $list )
				: $list;
...
if ( ! function_exists( 'trx_addons_get_list_sc_cover_places' ) ) {
	...
	function trx_addons_get_list_sc_cover_places() {
		return apply_filters( 'trx_addons_filter_get_list_sc_cover_places', array(
			...
			'p1' 	 => esc_html__( 'Parent', 'trx_addons' ),
			'p2'	 => esc_html__( 'Second-level parent', 'trx_addons' ),
			'p3'	 => esc_html__( 'Third-level parent', 'trx_addons' ),
		) );
	}
}
...
if ( ! function_exists( 'trx_addons_get_list_sc_slider_controls_paginations_types' ) ) {
	...
	function trx_addons_get_list_sc_slider_controls_paginations_types() {
		return apply_filters( 'trx_addons_filter_get_list_sc_slider_controls_paginations_types', array_merge(
						...
						apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'widget_slider' ) ? array( 'titles' => esc_html__( 'Titles', 'trx_addons' ) ) : array(),
