	...
	function trx_addons_get_share_links( $args, $list = false ) {
		...
		if ( is_array( $list ) ) {
			foreach ( $list as $social ) {
				...
				} else if ( trx_addons_is_on( trx_addons_get_option( 'add_target_blank_own', 0 ) ) ) {
				...
				foreach( $url['attributes'] as $k => $v ) {
					$v = str_replace(
									...
									array(
										...
										$k == 'href' && ! $email ? urlencode( strip_tags( $args['post_title'] ) ) : rawurlencode( html_entity_decode( strip_tags( $args['post_title'] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ),
										$k == 'href' && ! $email ? urlencode( strip_tags( $args['post_descr'] ) ) : rawurlencode( html_entity_decode( strip_tags( $args['post_descr'] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ),
							...
							. ( ! $show_names || $show_icons
									...
									. '>'
										. ( $show_icons || $show_names
											? ( $show_icons
												? ( ! $show_names ? '<span class="screen-reader-text">' . esc_html( ! empty( $url['attributes']['title'] ) ? $url['attributes']['title'] : ( ! empty( $title ) ? $title : $fn ) ) . '</span>' : '' )
		...
		if ( ! empty( $output ) ) {
			$output = $args['before']
						...
						. '">'
							. ( $args['caption'] != '' 
								? ( $args['type'] == 'drop' 
									? '<a href="#" role="button" class="socials_caption"><span class="socials_caption_label">' . $args['caption'] . '</span></a>'
	...
	function trx_addons_get_socials_links_custom( $icons, $style = '', $show = 'icons' ) {
		...
		if ( is_array( $icons ) && ! empty( $icons[0] ) ) {
			foreach ( $icons as $social ) {
				...
				$output .= '<a' . trx_addons_external_links_target() . ' href="' . ( strpos( $url, 'mailto:' ) !== false || strpos( $url, 'skype:' ) !== false
															? esc_attr( $url )
															: esc_url( $url )
															)
														. '"'
							...
							. ($show_icons
								? '<span class="social_icon social_icon_' . esc_attr( $fn) . ( ! empty( $color ) ? ' social_icon_colored' : '' ) . '"'
									. ' style="'
										...
										. '"'
									. '>'
										. ( ! $show_names ? '<span class="screen-reader-text">' . esc_html( ! empty( $title ) ? $title : $fn ) . '</span>' : '' )
										. ( $style == 'icons' 
											? '<span class="' . esc_attr( $sn ) . '"></span>' 
											: ( $style == 'svg'
												? trx_addons_get_svg_from_file( $sn )
...
if ( ! function_exists( 'trx_addons_add_og_tags' ) ) {
	...
	function trx_addons_add_og_tags() {
		...
		if ( trx_addons_is_singular() && ( ! isset( $wp_query->is_posts_page ) || $wp_query->is_posts_page != 1 ) && ! is_home() && ! is_front_page() && ! empty( $wp_query->post ) ) {
			...
			<?php
			$logo = apply_filters( 'trx_addons_filter_theme_logo', '', 'default', 'socials' );
			if ( is_array( $logo ) ) {
