...
if ( ! function_exists( 'trx_addons_validate_html_tag' ) ) {
	/**
	 ...
	 */
	function trx_addons_validate_html_tag( $tag ) {
		// Check if Elementor method exists, else we will run custom validation code.
		if ( trx_addons_exists_elementor() && method_exists( '\Elementor\Utils', 'validate_html_tag' ) ) {
...
if ( ! function_exists( 'trx_addons_external_links_target' ) ) {
	/**
	 * Return target="_blank" attribute for the external links if an option 'add_target_blank_own' is enabled.
	 *
	 ...
	 */
	function trx_addons_external_links_target( $admin = false ) {
		$attr = '';
		if ( $admin || trx_addons_is_on( trx_addons_get_option( 'add_target_blank_own', 0 ) ) ) {
...
if ( ! function_exists( 'trx_addons_get_site_domain' ) ) {
	/**
	 * Return a site domain url, used for the theme activation
	 * 
...
if ( ! function_exists( 'trx_addons_add_to_url' ) ) {
	...
	function trx_addons_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			// Way 1: use http_build_query (add indexes to the array params instead of empty brackets)
			// $url = $parts[0] . ( count( $params ) > 0 ? '?' . http_build_query( $params ) : '' );
			// Way 2: build query manually (to use brackets for array params)
			$url = $parts[0];
			$separator = '?';
			foreach( $params as $k => $v ) {
				if ( is_array( $v ) ) {
					if ( isset( $v[0] ) ) {
						// Indexed array
						foreach( $v as $kk => $vv ) {
							$url .= $separator . urlencode( $k ) . '[]=' . urlencode( $vv );
							$separator = '&';
						}
					} else {
						// Associative array
						foreach( $v as $kk => $vv ) {
							$url .= $separator . urlencode( $k ) . '[' . urlencode( $kk ) . ']=' . urlencode( $vv );
							$separator = '&';
						}
					}
				} else {
