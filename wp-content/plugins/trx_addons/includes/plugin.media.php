...
//			'parallax' => 0,				// Parallax shift (in %)
		...
		if ( empty($images) || ! is_array($images) ) {
			...
			foreach ( $images as $k=>$v ) {
				...
				$images[$k]['link_atts'] = trx_addons_get_link_attributes( $v );
			}
		}
...
//											: '' )
			...
			$allow_sc_styles_in_elementor = apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'sc_slider' );
			$arrow_prev = ! empty( $allow_sc_styles_in_elementor ) ? '' : ( ! empty( $args['controls_icon'] ) ? trim( $args['controls_icon'] ) : 'slider_arrow_default' );
			$arrow_next = ! empty( $allow_sc_styles_in_elementor ) ? '' : ( ! empty( $arrow_prev ) ? str_replace( 'left', 'right', $arrow_prev ) : '' );
			$controls_output = trx_addons_is_on( $args['controls'] )
								? apply_filters( 'trx_addons_filter_slider_controls',
									'<div class="slider_controls_wrap slider_arrows_wrap">'
										. '<a class="slider_prev ' . esc_attr( $arrow_prev ) . ' ' . esc_attr( $args['engine'] ) . '-button-prev" href="#" role="button">'
											. ( $allow_sc_styles_in_elementor && ! empty( $args['controls_icon_prev'] )
													? '<span class="slider_prev_icon trx-addons-icon">'
														. trx_addons_elm_render_icon( $args['controls_icon_prev'], 'eicon-chevron-left', false )
														. '</span>'
													: ''
													)
											. ( $args['style'] == 'modern' && ! empty( $args['label_prev'] ) 
												...
												: '' )
										. '</a>'
										. '<a class="slider_next ' . esc_attr( $arrow_next ) . ' ' . esc_attr( $args['engine'] ) . '-button-next" href="#" role="button">'
											. ( $allow_sc_styles_in_elementor && ! empty( $args['controls_icon_next'] )
													? '<span class="slider_next_icon trx-addons-icon">'
														. trx_addons_elm_render_icon( $args['controls_icon_next'], 'eicon-chevron-right', false )
														. '</span>'
													: ''
													)
			...
			if ( trx_addons_is_on( $args['controller'] ) ) {
				$arrow_prev = ! empty( $allow_sc_styles_in_elementor ) ? '' : ( ! empty( $args['controller_icon'] ) ? trim( $args['controller_icon'] ) : 'slider_arrow_default' );
				$arrow_next = ! empty( $allow_sc_styles_in_elementor ) ? '' : ( ! empty( $arrow_prev ) ? str_replace( array( 'left', 'up' ), array( 'right', 'down' ), $arrow_prev ) : '' );
				$output .= 	'<div'
				...
				$output .= 				'</div>'
								...
								. ( trx_addons_is_on( $args['controller_controls'] )
									? '<div class="slider_controls_wrap slider_controller_arrows_wrap">'
											. '<a class="slider_prev ' . esc_attr( $arrow_prev ) . ' swiper-button-prev" href="#" role="button">'
												. ( $allow_sc_styles_in_elementor && ! empty( $args['controller_icon_prev'] )
														? '<span class="slider_prev_icon trx-addons-icon">'
															. trx_addons_elm_render_icon( $args['controller_icon_prev'], 'eicon-' . ( $args['controller_pos'] == 'bottom' ? 'chevron-left' : 'sort-up' ), false )
															. '</span>'
														: ''
														)
											. '</a>'
											. '<a class="slider_next ' . esc_attr( $arrow_next ) . ' swiper-button-next" href="#" role="button">'
												. ( $allow_sc_styles_in_elementor && ! empty( $args['controller_icon_next'] )
														? '<span class="slider_next_icon trx-addons-icon">'
															. trx_addons_elm_render_icon( $args['controller_icon_next'], 'eicon-' . ( $args['controller_pos'] == 'bottom' ? 'chevron-right' : 'sort-down' ), false )
															. '</span>'
														: ''
														)
											. '</a>'
...
if ( ! function_exists( 'trx_addons_mediaplayer_icons_selector_allowed' ) ) {
	/**
	 * Return true if icons selector is allowed for the media player
	 *
	 * @return bool  True if icons selector is allowed, false otherwise
	 */
	function trx_addons_mediaplayer_icons_selector_allowed() {
		return apply_filters( 'trx_addons_filter_mediaplayer_icons_selector_allowed', false )								// Disable icons selector by default
			|| ! apply_filters( 'trx_addons_filter_allow_sc_styles_in_elementor', false, 'mediaplayer_icons_selector' );	// Enable icons selector for old themes
	}
}

if ( ! function_exists( 'trx_addons_mediaplayer_add_button_icons' ) ) {
	/**
	 * Return a data-parameter attributes with icons for each control button
	 *
	 * @param array  $args   Additional parameters with keys 'icon_media_play', 'icon_media_volume', etc.
	 * 
	 * @return string        String with data-parameter attributes
	 */
	function trx_addons_mediaplayer_add_button_icons( $args ) {
		$buttons = array( 'icon_media_play', 'icon_media_pause', 'icon_media_volume', 'icon_media_mute', 'icon_media_fullscreen', 'icon_media_unfullscreen' );
		$output = '';
		foreach ( $buttons as $button ) {
			if ( ! empty( $args[ $button ] ) ) {
				if ( trx_addons_exists_elementor() && is_array( $args[ $button ] ) && ! empty( $args[ $button ]['value'] ) ) {
					$icon = \Elementor\Icons_Manager::try_get_icon_html( $args[ $button ], [ 'aria-hidden' => 'true' ], 'span' );
				} else if ( is_string( $args[ $button ] ) ) {
					$icon = '<span class="' . esc_attr( $args[ $button ] ) . '" aria-hidden="true"></span>';
				} else {
					$icon = '';
				}
				if ( ! empty( $icon ) ) {
					$output .= ' data-' . str_replace( '_', '-', $button ) . '="' . esc_attr( $icon ) . '"';
				}
			}
		}
		return $output;
...
if ( ! function_exists('trx_addons_get_svg_from_file' ) ) {
	...
	function trx_addons_get_svg_from_file( $svg ) {
		$svg = trx_addons_is_url( $svg )
				? esc_url_raw( $svg )				// Sanitize URL
				: trx_addons_esc( $svg, true );		// Sanitize file path
		// Check if the file is a valid SVG file
		if ( trx_addons_get_file_ext( $svg ) !== 'svg' ) {
			return '';
		}
		// If the file is a URL, convert it to a local path if possible
		...
		if ( preg_match( "#<\s*?svg\b[^>]*>(.*?)</svg\b[^>]*>#is", $content, $matches ) ) {
			$content = ! empty( $matches[0]) ? str_replace( array( "\r", "\n" ), array( '', ' ' ), $matches[0] ) : '';
			if ( ! empty( $content ) && strpos( $content, '<svg' ) !== false ) {
				$content = trx_addons_sanitize_svg( $content );
			}
		} else {
			$content = '';
		}
...
if ( ! function_exists('trx_addons_sanitize_svg' ) ) {
	/**
	 ...
	 * 
	 * @param string $content  Content of the SVG file
	 * 
	 * @return string  Sanitized SVG content
	 */
	function trx_addons_sanitize_svg( $content ) {
		if ( ! empty( $content ) && is_string( $content ) ) {
			// Remove disallowed tags that could be used for XSS attacks
			$disallowed_tags = array( 'script', 'foreignobject', 'object', 'iframe', 'applet', 'embed', 'form', 'input', 'textarea', 'select', 'button' );
			...
			$content = preg_replace( "#\s*src='[^']*'#is", '', $content ); // Remove src attributes
			$content = preg_replace( '#\s*on\w+="[^"]*"#is', '', $content ); // Remove event handlers like onclick, onmouseover, etc.
			$content = preg_replace( "#\s*on\w+='[^']*'#is", '', $content ); // Remove event handlers like onclick, onmouseover, etc.
			$content = preg_replace( '#javascript:#is', '', $content ); // Remove javascript: protocol
			$content = preg_replace( '#\s*xmlns:[a-zA-Z]+="[^"]*"#is', '', $content ); // Remove XML namespaces
			$content = preg_replace( "#\s*xmlns:[a-zA-Z]+='[^']*'#is", '', $content ); // Remove XML namespaces
			$content = preg_replace( '#\s*xmlns="[^"]*"#is', '', $content ); // Remove default XML namespace
			$content = preg_replace( "#\s*xmlns='[^']*'#is", '', $content ); // Remove default XML namespace
		}
		return $content;
	}
