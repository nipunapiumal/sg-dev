...
if ( ! function_exists( 'trx_addons_current_user_can_edit' ) ) {
	/**
	 * Check if the current user can edit posts/pages.
	 *
	 * @param int $post_id        A post ID to check the permissions.
	 * @param string $post_type   Optional. A post type to check the permissions.
	 * @param string $capability  Optional. A default capability to check. Default is 'post'.
	 * 
	 * @return bool               Return true if the current user can edit the posts/pages.
	 */
	function trx_addons_current_user_can_edit( $post_id, $post_type = '', $capability = 'post' ) {
		if ( empty( $post_type ) ) {
		...
		return current_user_can( 'edit_' . $capability, $post_id );
			// || current_user_can( 'edit_' . $capability . 's' )
			// || current_user_can( 'administrator' );
	}
}

if ( ! function_exists( 'trx_addons_get_list_roles' ) ) {
	/**
	 * Return list of available user roles
	 * 
	 * @return array  Array of user roles
	 */
	function trx_addons_get_list_roles() {
		global $wp_roles;
		$roles = array();
