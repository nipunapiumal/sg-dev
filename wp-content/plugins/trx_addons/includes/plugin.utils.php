...
if ( ! function_exists( 'trx_addons_strshort' ) ) {
	 ...
	 */
	function trx_addons_strshort( $str, $maxlength, $add='&hellip;', $by_words = true ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
...
if ( ! function_exists( 'trx_addons_unserialize' ) ) {
	 ...
	 */
	function trx_addons_unserialize( $str, $disable_objects = false ) {
...
if ( ! function_exists( 'trx_addons_decode_settings' ) ) {
	...
	function trx_addons_decode_settings( $args ) {
		...
		return trx_addons_unserialize( $args );
	}
}
...
if ( ! function_exists('trx_addons_get_unique_id') ) {
	/**
	 * Return unique ID as a hex string with length of 7 characters
	 * 
...
if ( ! function_exists('trx_addons_get_link_attributes') ) {
	/**
	 * Return a string with a link attributes from the shortcode (widget) parameters.
	 * 
	 * @param array $atts        array with a shortcode (widget) parameters
	 * @param string $link_name  name of the parameter with a link (default is 'link')
	 * 
	 * @return string            string with a link attributes
	 */
	function trx_addons_get_link_attributes( $args, $link_name = 'link' ) {
		if ( substr( $link_name, -6 ) != '_extra' ) {
			$link_name = $link_name . '_extra';
		}
		$rez = ( ! empty( $args['new_window'] ) || ! empty( $args[ $link_name ]['is_external'] ) ? ' target="_blank"' : '' )
			. ( ! empty( $args['nofollow'] ) || ! empty( $args[ $link_name ]['nofollow'] ) ? ' rel="nofollow"' : '' );
		if ( ! empty( $args[ $link_name ]['custom_attributes'] ) && strpos( $args[ $link_name ]['custom_attributes'], '|' ) > 0 ) {
			$pairs = explode( ',', $args[ $link_name ]['custom_attributes'] );
			foreach ( $pairs as $v ) {
				$pair = explode( '|', $v );
				if ( count( $pair ) == 2 ) {
					$rez .= ' ' . $pair[0] . '="' . esc_attr( $pair[1] ) . '"';
				}
			}
		}
		return $rez;
