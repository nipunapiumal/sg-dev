...
if ( ! function_exists( 'trx_addons_localize_scripts_admin' ) ) {
	...
	function trx_addons_localize_scripts_admin() {
		...
		wp_localize_script( 'trx_addons-admin', 'TRX_ADDONS_STORAGE', apply_filters('trx_addons_filter_localize_script_admin', array(
			...
			// Maximum count of variables passed in query
			...
			'msg_extend_license'			=> addslashes(esc_html__( 'Extend License', 'trx_addons' )),
			'msg_extend_license_agree'		=> addslashes(esc_html__( 'You will be redirected to the payment page to renew your license. After payment, refresh the current page to update the support date.', 'trx_addons' )),
			'msg_extend_license_bt_yes'		=> addslashes(esc_html__( 'Extend', 'trx_addons' )),
			'msg_extend_license_bt_yes2'	=> addslashes(esc_html__( 'Get link', 'trx_addons' )),
			'msg_extend_license_bt_no'		=> addslashes(esc_html__( 'Cancel', 'trx_addons' )),
			'msg_check_license'				=> addslashes(esc_html__( 'Check License', 'trx_addons' )),
			...
			'msg_max_input_vars'			=> addslashes(esc_html__( 'The total number of fields in all menu items ( {{count}} ) exceeds the maximum number of fields in the request ( {{max_input_vars}} ) specified in your server settings.<br>Please increase the limit in your server configuration (an option "max_input_vars" in the file "php.ini") or reduce the number of menu items.<br>Otherwise, you may lose data when you save the menu!', 'trx_addons' )),
			'msg_legacy_theme_caption'		=> addslashes(esc_html__( 'Legacy Theme Version', 'trx_addons' )),
			'msg_legacy_theme'				=> addslashes(esc_html__( 'Your theme isn\'t on the latest version yet. Please go to Dashboard > Updates to update your theme first. Once the theme is updated, you\'ll be able to download and update skins.', 'trx_addons' )),
			'msg_update_theme'				=> addslashes(esc_html__( 'Theme Updating', 'trx_addons' )),
			'msg_update_theme_try_again'	=> addslashes(esc_html__( 'Please, reload this page and try again.', 'trx_addons' )),
		) ) );
	}
}
