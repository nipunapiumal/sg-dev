(function () {
					...
					if (widgets[i].contains(e.target)) {
						...
						if (icon.classList.toString().indexOf("pa-pro") >= 0) {
							...
							var buttonText = document.createTextNode(
								"Upgrade Premium Addons (Save 30%)",
							);

							button.setAttribute(
								"href",
								PremiumSettings.upgrade_link +
									"&utm_source=panel-" +
									proElement,
							);
	...
	function onSectionActivate(sectionName) {
		if ("access_credentials_section" === sectionName) {
			setTimeout(function () {
				var accessToken = jQuery(
					".elementor-control-access_token textarea",
				).val();
	...
	var selectOptions = elementor.modules.controls.Select2.extend({
		...
		onReady: function () {
			...
			if (
				this.container &&
				"widget" === this.container.type &&
				"board_id" === this.model.get("name")
			) {
				if (options) {
					var _this = this;
		...
		onBeforeRender: function () {
			if (
				this.container &&
				("section" === this.container.type ||
					"container" === this.container.type)
			) {
					...
					if ("container" === _this.container.type) {
						if (child.view.$childViewContainer) {
							...
							if ("undefined" !== typeof widgetObj[name]) {
								optionsToUpdate[
									".elementor-widget-" +
										widgetObj[name].widget_type +
										" .elementor-widget-container"
								] = widgetObj[name].title;
							}
						}
				...
				function getInnerWidgets(child) {
					child.view.$childViewContainer
						.children("[data-widget_type]")
	...
	var filterOptions = elementor.modules.controls.Select2.extend({
		...
		onReady: function () {
			var self = this,
				type =
					self.model.get("source") ||
					self.options.elementSettingsModel.attributes.post_type_filter;
			...
			elementor.channels.editor.on("change", function (view) {
				...
				if (
					undefined !== changed.post_type_filter &&
					"post" !== changed.post_type_filter &&
					!self.isUpdated
				) {
					self.isUpdated = true;
				...
				if (view.$el.hasClass("premium-live-temp-label")) {
					...
					if (
						undefined !== changed[view.model.get("name")] &&
						"" !== changed[view.model.get("name")]
					) {
			...
			$.ajax({
				...
				success: function (res) {
					...
					if (0 !== options.length) {
						...
						$(".premium-live-temp-title.control-hidden").removeClass(
							"control-hidden",
						);
					}
				},
		...
		imageChoose = e.extend(
			{
				...
				onMouseDownLabel: function (e) {
					...
					$(".elementor-control-form_insert .elementor-button").css(
						"background-color",
						"#252c59",
					);
				...
				onReady: function () {
					if ("premium_gdivider_defaults" === this.model.attributes.name) {
						const choicesContainer = $(this.el).find(
